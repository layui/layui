...
Class.prototype.stampRange = function () {
  ...
  lay.each(tds, function (i, item) {
    ...
    if (thisTime === startTime || thisTime === endTime) {
      (that.rangeLinked || !that.rangeLinked && (i < 42 ? thisTime === startTime : thisTime === endTime)) && lay(item).addClass(lay(item).hasClass(ELEM_PREV) || lay(item).hasClass(ELEM_NEXT) ? ELEM_SELECTED : THIS);
    }
    if (thisTime > startTime && thisTime < endTime) {
