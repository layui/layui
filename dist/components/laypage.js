...
Class.prototype.view = function () {
  ...
  var views = {
    // 上一页
    prev: function () {
      return config.prev ? '<a class="layui-laypage-prev' + (config.curr == 1 ? ' ' + DISABLED : '') + '" data-page="' + (config.curr - 1) + '">' + config.prev + '</a>' : '';
    }(),
    // 页码
    page: function () {
      var pager = [];
    ...
    // 下一页
    next: function () {
      return config.next ? '<a class="layui-laypage-next' + (config.curr == config.pages ? ' ' + DISABLED : '') + '" data-page="' + (config.curr + 1) + '">' + config.next + '</a>' : '';
    }(),
    // 数据总数
    count: function () {
