...
var component = component$1({
  ...
  render: function () {
    ...
    // 渲染成功后的事件
    layui.event.call(options.elem[0], component.CONST.MOD_NAME, 'afterRender(' + options.id + ')', data);
  },
  // 事件
  events: function () {
    var that = this;
...
Class.prototype.close = function (thisHeaderItem, force) {
  ...
  if (!force) {
    var closable = layui.event.call(thisHeaderItem[0], component.CONST.MOD_NAME, 'beforeClose(' + options.id + ')', $.extend(data, {
      index: index
    }));
  ...
  // 标签关闭后的事件
  layui.event.call(data.thisHeaderItem[0], component.CONST.MOD_NAME, 'afterClose(' + options.id + ')', data);
};
...
Class.prototype.change = function (thisHeaderItem, force) {
  ...
  // 标签关闭前的事件。若非强制关闭，可则根据事件的返回结果决定是否关闭
  if (!force) {
    var enable = layui.event.call(thisHeaderItem[0], component.CONST.MOD_NAME, 'beforeChange(' + options.id + ')', $.extend(data, {
      from: {
        index: data.index,
  ...
  // 执行标签头部切换
  thisHeaderItem.addClass(component.CONST.CLASS_THIS).siblings().removeClass(component.CONST.CLASS_THIS);

  // 执行标签内容切换
  that.findBodyItem(layid || index).addClass(component.CONST.CLASS_SHOW).siblings().removeClass(component.CONST.CLASS_SHOW);
  that.roll('auto', index);
  ...
  // 标签切换后的事件
  layui.event.call(data.thisHeaderItem[0], component.CONST.MOD_NAME, 'afterChange(' + options.id + ')', data);
};
...
Class.prototype.roll = function (mode, index) {
  ...
  // 左
  elemScroll.find('.' + CLASS_BAR_ICON[0])[tabsLeft < 0 ? 'removeClass' : 'addClass'](component.CONST.CLASS_DISABLED);
  // 右
  elemScroll.find('.' + CLASS_BAR_ICON[1])[parseFloat(tabsLeft + scrollWidth) - outerWidth > 0 ? 'removeClass' : 'addClass'](component.CONST.CLASS_DISABLED);
};
