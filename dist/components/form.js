...
Form.prototype.render = function (type, filter) {
  ...
  if (layui.type(type) === 'object') {
    ...
    type ? items[type] ? items[type]() : hint.error('[form] "' + type + '" is an unsupported form element type') : renderItem();
  }
  return that;
};
...
var submit = Form.prototype.submit = function (filter, callback) {
  ...
  // 当前所在表单域
  var elem = this.getFormElem ? this.getFormElem(layFilter) : button.parents(ELEM).eq(0);
