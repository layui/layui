...
Class.prototype.slide = function (setValue, value, i) {
  ...
  var change = function (offsetValue, index, from) {
    ...
    if (options.type === 'vertical') {
      sliderAct.find('.' + CONST.SLIDER_TIPS).css({
        bottom: offsetValue + '%',
        'margin-bottom': '20px'
      });
    ...
    if (options.type === 'vertical') {
      sliderAct.find('.' + CONST.SLIDER_BAR).css({
        height: wrapWidth + '%',
        bottom: minLeft + '%'
      });
    } else {
