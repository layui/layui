...
layui.define("jquery", function (e) {
    ...
    var i = layui.jquery,
        ...
        r = "layui-slider-bar",
        o = "layui-slider-wrap",
        d = "layui-slider-wrap-btn",
        u = "layui-slider-tips",
        c = "layui-slider-input",
        v = "layui-slider-input-txt",
    ...
    f.prototype.config = {
        ...
        if (t.min = t.min < 0 ? 0 : t.min, t.range) {
            ...
            var r = Math.floor((t.value[0] - t.min) / (t.max - t.min) * 100),
                c = Math.floor((t.value[1] - t.min) / (t.max - t.min) * 100),
                m = c - r + "%";
            r += "%", c += "%"
        } else {
        ...
        var n = this,
            ...
            h = s.find("." + o),
            y = s.next("." + c),
            g = y.children("." + v).children("input").val(),
            ...
            d = {
                select: function () {
                    ...
                    f.each(function (e, l) {
                        ...
                        /* 处理下拉框require无效 */
                        var ir = (l.getAttribute('required') !== null && l.getAttribute('required') !== false);
                        var re = (true === ir) ? 'required="required"' : '';
                        var h = "string" == typeof r.attr("lay-search"),
                            p = v ? v.value ? t : v.innerHTML || t : t,
                            m = i(['<div class="' + (h ? "" : "layui-unselect ") + a, (u ? " layui-select-disabled" : "") + '">', '<div class="' + n + '">', '<input type="text" ' + re + ' placeholder="' + p + '" title="' + p + '" ' + ('value="' + (d ? f.html() : "") + '"') + (h ? "" : " readonly") + ' class="layui-input' + (h ? "" : " layui-unselect") + (u ? " " + c : "") + '">', '<i class="layui-edge"></i></div>', '<dl class="layui-anim layui-anim-upbit' + (r.find("optgroup")[0] ? " layui-select-group" : "") + '">',
                                function (e) {
                                    var i = [];
    ...
    var t = layui.$,
        ...
        S = function (e) {
            ...
            return e = e || {}, ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', "<thead>", "{{# layui.each(d.data.cols, function(i1, item1){ }}", "<tr>", "{{# layui.each(item1, function(i2, item2){ }}", '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}',
                ...
                }(), "{{# var isSort = !(item2.colspan > 1) && item2.sort; }}", '<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' + t + ' {{# if(item2.unresize){ }}data-unresize="true"{{# } }} {{# if(isSort){ }}class="layui-unselect"{{# } }}>', '<div class="layui-table-cell laytable-cell-', "{{# if(item2.colspan > 1){ }}", "group", "{{# } else { }}", "{{d.index}}-{{item2.field || i2}}", '{{# if(item2.type !== "normal"){ }}', " laytable-cell-{{ item2.type }}", "{{# } }}", "{{# } }}", '" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}', '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', "{{# } else { }}", '<span>{{item2.title||""}}</span>', "{{# if(isSort){ }}", '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="升序"></i><i class="layui-edge layui-table-sort-desc" title="降序"></i></span>', "{{# } }}", "{{# } }}", "</div>", "</th>", e.fixed ? "{{# }; }}" : "", "{{# }); }}", "</tr>", "{{# }); }}", "</thead>", "</table>"
            ].join("")
        },
