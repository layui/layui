...
异步按钮。开启之后，除 `layer.prompt` 的按钮外，按钮回调的返回值将支持 `boolean | Promise<boolean> | JQueryDeferred<boolean>` 类型，返回 false 或 Promise.reject 时阻止关闭。注意，此时 yes 和 btn1(两者等效) 回调的默认行为发生了变化，由触发时不关闭弹层变为关闭弹层。
</div>
