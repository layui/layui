...
layui.define(
  ['lay', 'i18n', 'laytpl', 'laypage', 'form', 'util'],
  function (exports) {
    'use strict';
    ...
    var parseTempData = function (obj) {
      ...
      if (item3.type === 'numbers')
        content = obj.tplData[table.config.numbersName];
     ...
     * ==================
     *
     * layui table 使用 laytpl 模板引擎来生成 HTML 结构，主要包含以下几个模板：
     *
     * 1. TPL_HEADER - 表头模板
     *    - 负责生成 <thead> 部分的 HTML
     *    - 支持多级表头、固定列、排序等功能
     *    - 可通过 options.fixed 参数控制是否为固定列模板
     *
     * 2. TPL_BODY - 表体模板
     *    - 负责生成 <tbody> 的容器结构
     *    - 实际的数据行由 JavaScript 动态生成
     *
     * 3. TPL_MAIN - 主布局模板
     *    - 整个表格组件的外层结构
     *    - 包含工具栏、表格主体、固定列、统计行、分页等区域
     *
     * 模板语法说明：
     * - {{# }} - 逻辑代码块
     * - {{= }} - 输出变量（会进行 HTML 转义）
    ...
    var TPL_HEADER = function (options) {
      // 合并单元格属性模板片段
      var rowCols = [
        "{{# var colspan = layui.type(item2.colspan2) === 'number' ? item2.colspan2 : item2.colspan; }}",
        '{{# if(colspan){ }} colspan="{{=colspan}}"{{# } }}',
        '{{# if(item2.rowspan){ }} rowspan="{{=item2.rowspan}}"{{# } }}'
      ].join('');

      options = options || {};

      // 固定列条件判断
      var fixedCondition = (function () {
      ...
      return [
        ...
        // CSS 类名
        '              class="{{# if(item2.hide){ }}layui-hide{{# } }}',
        '                     {{# if(isSort){ }} layui-unselect{{# } }}',
        '                     {{# if(!item2.field){ }} layui-table-col-special{{# } }}"',
        ...
        '                       {{# if(item2[d.data.checkName]){ }} checked{{# } }}>',
        ...
        '                       title="{{= d.i18nMessages.table_sort_asc }}"></i>',
        '                    <i class="layui-edge layui-table-sort-desc"',
        '                       title="{{= d.i18nMessages.table_sort_desc }}"></i>',
        '                  </span>',
        '                {{# } }}',
        '              {{# } }}',
      ...
      var defaultConfig = {
        ...
        exports: {
          ...
          onClick: function (obj) {
            ...
            if (!data.length)
              return layer.tips(
                i18n.$t('table.tools.export.noDataPrompt'),
                elem,
                { tips: 3 }
              );
            if (device.ie) {
        ...
        print: {
          ...
          onClick: function (obj) {
            ...
            if (!data.length)
              return layer.tips(
                i18n.$t('table.tools.print.noDataPrompt'),
                elem,
                { tips: 3 }
              );
      ...
      if (lastSpreadCol) {
        that.cssRules(lastSpreadCol.key, function (item) {
          ...
          var newWidth = Math.max(
            Math.min(pixelsForLastCol, maxWidth),
            minWidth
          );
          item.style.width = newWidth + 'px';
          ...
          if (
            that.layMain.prop('offsetHeight') >
            that.layMain.prop('clientHeight')
          ) {
            item.style.width =
              parseFloat(item.style.width) - borderWidth + 'px';
          }
        });
      }
    ...
    Class.prototype.resize = function (entry) {
      ...
      if(entry){
        var shouldIgnore = entry.target._lay_lastSize &&
    ...
    Class.prototype.renderData = function (opts) {
      ...
      //显示隐藏分页栏
      that.layPage[
        options.page || options.pagebar ? 'removeClass' : 'addClass'
      ](HIDE);
      that.layPage
        .find(ELEM_PAGE_VIEW)
        [
    ...
    Class.prototype.renderTotal = function (data, totalRowData) {
      ...
      layui.each(data, function (i1, item1) {
        ...
        that.eachCols(function (i3, item3) {
          ...
          if (item3.totalRow) {
            totalNums[field] =
              (totalNums[field] || 0) + (parseFloat(content) || 0);
          }
        });
      });
    ...
    Class.prototype.events = function () {
      ...
      var renderGridEdit = function (othis, e) {
        ...
        var editType =
          typeof col.edit === 'function' ? col.edit(data) : col.edit;
      ...
      var gridExpand = function (e, expandedMode) {
        ...
        expandedMode =
          expandedMode || col.expandedMode || options.cellExpandedMode;
        ...
        if (expandedMode === 'tips') {
          ...
          that.tipsIndex = layer.tips(
            ...
            elemCell[0],
            {
              ...
              anim: -1,
              maxWidth:
                device.ios || device.android ? 300 : that.elem.width() / 2,
              isOutAnim: false,
