...
layui.define(['i18n', 'component'], function(exports) {
  ...
  var component = layui.component({
    ...
    render: function() {
      ...
      // 颜色选择框对象
      var elemColorBox = $(that.buildColorBoxTemplate(options));
  ...
  // 构建颜色选择框模板
  Class.prototype.buildColorBoxTemplate = function(options) {
    ...
    return [
      ...
      '  <span ' + (options.format == 'rgb' && options.alpha ? 'class="layui-colorpicker-trigger-bgcolor"' : '') + '>',
      '    <span class="layui-colorpicker-trigger-span"',
      '          lay-type="' + (options.format == 'rgb' ? (options.alpha ? 'rgba' : 'torgb') : '') + '"',
      '          style="' + getBackgroundStyle() + '">',
      '      <i class="layui-icon layui-colorpicker-trigger-i ' + (options.color ? CONST.ICON_PICKER_DOWN : CONST.ICON_PICKER_CLOSE) + '"></i>',
      '    </span>',
      '  </span>',
      '</div>',
    ].join('');
  ...
  // 构建颜色选择器模板
  Class.prototype.buildColorPickerTemplate = function(options, that) {
    var buildPredefineColors = function() {
      if (options.predefine) {
        var list = ['<div class="layui-colorpicker-main-pre">'];
        layui.each(options.colors, function(i, v) {
          list.push([
            '<div class="layui-colorpicker-pre' + ((v.match(/[0-9]{1,3}/g) || []).length > 3 ? ' layui-colorpicker-pre-isalpha' : '') + '">',
    ...
    return [
      '<div id="layui-colorpicker' + that.index + '" data-index="' + that.index + '" class="layui-anim layui-anim-downbit layui-colorpicker-main">',
  ...
  Class.prototype.renderPicker = function() {
    ...
    // 颜色选择器对象
    var elemPicker = that.elemPicker = $(that.buildColorPickerTemplate(options, that));
  ...
  Class.prototype.side = function() {
    var that = this
      ...
      //拖拽元素
      , elemMove = $('<div class="layui-auxiliar-moving" id="LAY-colorpicker-moving"></div>')
