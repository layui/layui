...
// 图片层
  layer.photos = function (options, loop, key) {
    ...
    var showRemove = ('showRemove' in photos && typeof photos.showRemove === 'boolean' && photos.showRemove) || false;
    var success = options.success;
    ...
    dict.removeByIndex = function (index) {
      if (!dict.isNumber(index) || index < 1 || index > data.length) {
        return;
      }
    ...
    dict.event = function (layero, index, that) {
      ...
      // 删除当前图片
      dict.main.find('.layui-layer-photos-remove-btn').on('click', function (event) {
        event.preventDefault();
        dict.removeByIndex(dict.imgIndex);
      });
      ...
      dict.index = layer.open($.extend({
        ...
        content: '<div class="layer-layer-photos-main">'
          ...
          + function () {
            ...
            // 底部栏
            if (options.footer) {
              let removeBtnHtml = showRemove ? '<a href="#" class="layui-layer-photos-remove-btn">删除</a>' : '';
