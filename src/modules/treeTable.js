...
layui.define(['table'], function (exports) {
  ...
  var expandNode = function (treeNode, expandFlag, sonSign, focus, callbackFlag) {
    ...
    //不允许禁止节点展开
    if (!treeOptions.view.enableDisabledNodeExpand && trData[options.disabledName]) return;
  ...
  treeTable.expandAll = function (id, expandFlag, isOneExpand) {
    ...
    if (!expandFlag) {
      ...
      //展开一个子点
      if (isOneExpand){
        tableView.find('.layui-table-box .layui-table-body.layui-table-main tbody tr[data-level="0"] .layui-table-tree-flexIcon:not(.layui-disabled) .layui-icon-triangle-r').each(function () {
          $(this).click();
        });
        return ;
      }
  ...
  Class.prototype.renderTreeTable = function (tableView, level, sonSign) {
    ...
    layui.each(tableView.find('td[data-field="' + nameKey + '"]'), function (index, item) {
      ...
      //父点状态禁止时状态同步子点
      if (treeOptions.view.enableDisabledNodeExpand && trData[LAY_PARENT_INDEX] !== ''){
      ...
      var flexIconElem = itemCell
        .html(['<div class="layui-inline layui-table-tree-flexIcon'+(!treeOptions.view.enableDisabledNodeExpand && trData[options.disabledName] ? ' layui-disabled' : '')+'" ',
          'style="',
