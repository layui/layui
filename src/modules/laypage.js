...
layui.define(function (exports) {
  ...
  Class.prototype.view = function () {
    ...
    var views = {
      ...
      // 刷新当前页
      refresh: ['<span data-page="' + config.curr + '" class="layui-laypage-refresh" title="重载当前页数据">', '<i class="layui-icon layui-icon-refresh"></i>', '</span>'].join(''),

      // 跳页区域
      skip: function () {
        var skipText = typeof config.skipText === 'object' ? config.skipText : ['到第', '页', '确定'];
  ...
  Class.prototype.jump = function (elem, isskip) {
    ...
    for (var i = 0, len = childs.length; i < len; i++) {
      if (childs[i].getAttribute("data-page")) {
  ...
  Class.prototype.onselect = function () {
    var config = this.config;
    if (config.onselect) {
      config.onselect(config.curr, config.limit);
    }
  }
  // 输入页数字控制
