...
layui.define(['lay', 'layer', 'util'], function (exports) {
  ...
  /**
   * @typedef {Object} LoadSelectOptions
   * @property {string} selector DOM选择器
   * @property {Object[]} data 数据
   * @property {string} [valueKey=id] 数据中value的key @default id
   * @property {string} [showKey=name] 数据中name的key @default name
   * @property {function} [getShow] 获取选项显示的值，如果有该接口则默认使用该方法获取显示值，否则使用nameKey来获取显示值
   * @property {any} [selectValue] 选中的值
   * @property {boolean} [needDefault=false] 是否需要默认选项 @default false
   * @property {string} [defaultValue=''] 默认选项的值 @default ''
   * @property {string} [defaultText=''] 默认选项的显示文本
   */
  /**
   * 加载下拉框
   * @param {LoadSelectOptions} options 加载下拉框的配置项
   */
  Form.prototype.loadSelect = function (options) {
    options.valueKey = options.valueKey || 'id';
    options.showKey = options.showKey || 'name';
    options.defaultValue = options.defaultValue || '';
    let dom = options.data.map(item => `<option value="${ item[options.valueKey] }" ${ item[options.valueKey] === options.selectValue ? 'selected ' : '' }>${ options.getShow ? options.getShow(item) : item[options.showKey] }</option>`).join(' ')
    let el = layui.$(options.selector);
    el.empty();
    if (options.needDefault) {
      el.append(`<option value="${ options.defaultValue }">${ options.defaultText }</option>`);
    }
    el.append(dom);
    this.render('select')
  }
