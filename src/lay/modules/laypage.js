...
layui.define(function (exports) {
    ...
    Class.prototype.view = function () {
        ...
        ,
            ...
            //首页
            first: function () {
                return '<a href="javascript:;" class="layui-laypage-first' + (config.curr == 1 ? ' ' + DISABLED : '') + '" data-page="' + (config.curr == 1 ? config.curr - 1 : 1) + '"  title="&#x9996;&#x9875;">' + (config.first || '首页') + '</a>';
            }()
            //上一页
            ...
            last: function () {
                //输出输出右分隔符 & 末页
                return '<a href="javascript:;" class="layui-laypage-last' + (config.curr == config.pages ? ' ' + DISABLED : '') + '" data-page="' + (config.curr == config.pages ? config.pages + 1 : config.pages) + '"  >' + (config.first || '尾页') + '</a>';
            }()
            //数据总数

            , count: '<span class="layui-laypage-count">共 ' + config.count + ' 条记录</span>'
            //当前页数

            , currpage: function () {
                return '<span class="layui-laypage-count">当前 <i>' + config.curr + '</i>/' + config.pages + ' 页</span>';
            }()
            //每页条数
        ...
        return ['<div class="layui-box layui-laypage layui-laypage-' + (config.theme ? /^#/.test(config.theme) ? 'molv' : config.theme : 'default') + '" id="layui-laypage-' + config.index + '">', function () {
            ...
            layui.each(config.layout, function (index, item) {
                if (views[item]) {
                    if (item == 'count' || item == 'currpage') {
                        showCurr.push(views[item]);
                    } else {
                        plate.push(views[item]);
                    }
                }
            });
            return '<div class=\'laypageFloat left\'>' + showCurr.join('') + '</div><div class=\'laypageFloat right\'>' + plate.join('') + '</div>';
            // return  plate.join('');
        }(), '</div>'].join('');
    };
