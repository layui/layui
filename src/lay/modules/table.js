...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    ,
        ...
        options = options || {};
        return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.showCols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}', function () {
        ...
        , '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', '{{# } else { }}', '<span>{{item2.title||""}}</span>', '{{# if(!(item2.colspan > 1) && item2.sort){ }}', '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc"></i><i class="layui-edge layui-table-sort-desc"></i></span>', '{{# } }}', '{{# } }}', '</div>', '</th>', options.fixed ? '{{# }; }}' : '', '{{# }); }}', '</tr>', '{{# }); }}', '</thead>', '</table>'].join('');
    }
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        ...
        render = function render() {
            ...
            layui.each(data, function (i1, item1) {
                    ...
                    var td = ['<td data-field="' + field + '" ' + function () {
                        ...
                        if (item3.toolbar) {
                            ...
                            $(item3.toolbar.btn).each(function (index, tool) {
                                toolBox += '<a data-tool=' + index + ' class="layui-btn layui-btn-primary layui-btn-xs">' + tool.name + '</a>';
                            });
                ...
                trs.push('<tr ' + trAttr + trColor + ' data-index="' + i1 + '">' + tds.join('') + '</tr>');
