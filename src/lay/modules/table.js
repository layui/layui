...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
  ...
  TPL_HEADER = function TPL_HEADER(options) {
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}', function() {
        ...
        return '';
      }(), '<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' + rowCols + ' {{# if(item2.unresize){ }}data-unresize="true"{{# } }} {{# if(item2.hidden){ }} style="display:none;"{{# } }}>', '<div class="layui-table-cell laytable-cell-', '{{# if(item2.colspan > 1){ }}', 'group', '{{# } else { }}', '{{d.index}}-{{item2.field || i2}}', '{{# if(item2.type !== "normal"){ }}', ' laytable-cell-{{ item2.type }}', '{{# } }}', '{{# } }}', '" {{#if(item2.align){}}align="{{item2.align}}"{{#  } else { }} align="center"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}' //复选框
  ...
  ,
  TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', '{{# if(d.data.toolbar){ }}', '<div class="layui-table-tool"></div>', '{{# } }}', '<div class="layui-table-box">', '{{# var left, right; }}', '<div class="layui-table-header">', TPL_HEADER(), '</div>', '<div class="layui-table-body layui-table-main">', TPL_BODY, '</div>', '{{# if(left){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', TPL_HEADER({
      fixed: true
    }), '</div>', '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '{{# if(right){ }}', '<div class="layui-table-fixed layui-table-fixed-r">', '<div class="layui-table-header">', TPL_HEADER({
      fixed: 'right'
    }), '<div class="layui-table-mend"></div>', '</div>', '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '</div>', '{{# if(d.data.page){ }}', '<div class="layui-table-page">', '<div id="layui-table-page{{d.index}}"></div>', '</div>', '{{# } }}', '<style>', '{{# layui.each(d.data.cols, function(i1, item1){', 'layui.each(item1, function(i2, item2){ }}', '.laytable-cell-{{d.index}}-{{item2.field||i2}}{ ', '{{# if(typeof item2.width == "string" || item2.minWidth){ }}', 'width: {{ item2.width || item2.minWidth}};', '{{#  } else { }}', 'width: 50px;', '{{# } }}', ' }', '{{# });', '}); }}', '</style>', '</div>'].join(''),
    _WIN = $(window),
    _DOC = $(document)
      ...
      render = function render() {
        ...
        layui.each(data, function(i1, item1) {
          ...
          that.eachCols(function(i3, item3) {
            ...
            //td内容
            var td = ['<td id="' + field +'"  data-field="' + field + '" ' + function() {
              var attr = [];
              var iAlign = item3.align == undefined ? "center" : item3.align;
              attr.push(' align="' + iAlign + '"'); //对齐方式
              if (item3.edit) attr.push('data-edit="' + item3.edit + '"'); //是否允许单元格编辑
              if (item3.field == 'operatBox') attr.push('class="operatBox"'); //判断是否为操作栏
              if (item3.openTip) attr.push('class="openTip"'); //判断是否可以显示Tip
              // if (item3.align) attr.push('align="' + item3.align + '"'); //对齐方式
              if (item3.templet) attr.push('data-content="' + content + '"'); //自定义模板
              if (item3.toolbar) attr.push('data-off="true"'); //自定义模板
              if (item3.event) attr.push('lay-event="' + item3.event + '"'); //自定义事件
              if (item3.style) attr.push('style="' + item3.style + '"'); //自定义样式
              if (item3.hidden) attr.push('style="display:none;border:none"'); //自定义样式
              ...
              if (item3.type == 'video') {
                return '<video style="height: 30px;width: 100%;" controls="controls" preload="auto" src="' + content + '"></video>';
              }

              //解析工具列模板
              if (item3.toolbar) {
                if(typeof item3.toolbar == 'object') {
                  var toolBox = '';
                  $(item3.toolbar.btn).each(function(index, tool) {
                    toolBox += '<a data-tool=' + index + ' class="layui-btn layui-btn-xs layui-btn-normal">' + tool.name + '</a>';
                  });
                  return laytpl(toolBox).render(tplData);
                } else{
                  // console.log($(item3.toolbar).html());
          ...
          trs.push("<tr data-id='"+item1.FID+"'"+ trColor +" data-index='"+i1+"'>"+tds.join("")+"</tr>");
