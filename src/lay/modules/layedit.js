...
layui.define(['layer', 'form'], function(exports){
	...
	var setIframe = function(editor, textArea, set){
		...
		//在选区插入内联元素
		,insertInline = function(tagName, attr, range,delContents){
		    console.log(range)
			...
			if(iframeDOM.selection){ //IE
				...
				if(delContents==undefined || delContents==true){
				...
				,image: function(range){
					...
					layui.use('upload', function(upload){
						...
						upload.render({
							...
							,before: function () {
								loadIndex = layer.load(2); //添加laoding,0-2两种方式
							}
							,multiple: true
							,allDone:function (allRes) {
								for(var i=allRes.successful-1;i>=0;i--){
									insertInline.call(iframeWin, 'img', {
										src: imageUrlMap[i+""]
										,alt: imageAltMap[i+""]
									}, range,false);
							...
							,done: function(res,index){
								if(res.code == 0){
									// index 字符串格式   1593398165589-0
									var subIndex = index.indexOf("-");
									if(subIndex>0){
										index = index.substr(subIndex+1);
									}
									imageUrlMap[index+""] = res.data.src;
									imageAltMap[index+""] = res.data.title;
								} else {
		...
		,tools = {
			...
			,image: '<i class="layui-icon layedit-tool-image" title="图片" layedit-event="image">&#xe64a;<input type="file" multiple name="file"></i>'
