...
Layui 文档采用 `Markdown + HTML + laytpl` 混合编写，其中 laytpl 为视图引擎（后面考虑更换为 ejs），支持导入文档子模版，格式示例：`{{- d.include("/{组件名}/detail/demo.md") }}`

### 创建文档

当新增组件时，必须在 `docs/` 中根据 `docs/.template/` 给定的模板为组件新增对应的完整文档，保存在 `docs/{组件名}/` 目录。文档模板介绍：

- `index.md`: 组件文档主文件，包含示例、API、属性等完整内容。当整体内容过大时，可将内容篇幅较大的碎片作为子模版放置在 `detail/` 目录，并导入对应的子模版即可。
- `detail/`: 该目录用于存放文档子模板。
  - `detail/demo.md`: 组件示例主模板。当整体内容过大时，可将示例内容拆分成多个子模版，保存在 `examples/` 目录，并导入对应的示例子模版即可，
...
- 基于 `.github/pull_request_template.md` 的结构输出完整 PR 文本：
  ...
  - **变化内容**：使用清晰条目描述关键改动，优先覆盖：
    ...
    - 改动了哪些模块或行为
    - 对用户或维护者的直接收益
    - 若关联 issue，请使用 `closes` / `fixes` / `resolves` 等关键词（如 `closes #123`）。
