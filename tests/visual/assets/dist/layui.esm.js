...
var getPath = function () {
  ...
  return config$3.dir = GLOBAL$1.dir || jsPath.substring(0, jsPath.lastIndexOf('/') + 1);
}();
...
// 内置模块
var builtinModules = config$3.builtin = {
  lay: 'lay',
  // 基础 DOM 操作
  layer: 'layer',
  // 弹层
...
// 获取配置及临时缓存信息
Class$g.prototype.cache = Object.assign(config$3, cache$1);
...
Class$g.prototype.config = function (options) {
  Object.assign(config$3, options);
  return this;
};
...
Class$g.prototype.use = function (mods, callback, exports, from) {
  ...
  if (!config$3.host) {
    config$3.host = (dir.match(/\/\/([\s\S]+?)\//) || ['//' + location$1.host + '/'])[0];
  }
  ...
  var pollCallback = function () {
    ...
    // 轮询模块加载完毕状态
    (function poll() {
      if (++timeout > config$3.timeout * 1000 / delay) {
      ...
      (isExternalModule ? layui[item] = window[modInfo.api] : cache$1.status[item]) ? onCallback() : setTimeout(poll, delay);
    })();
  };
  ...
  // 基础路径
  var basePath = builtinModules[item] ? dir + 'modules/' // 若为内置模块，则按照默认 dir 参数拼接模块 URL
  : modSrc ? '' : config$3.base; // 若为扩展模块，且模块路径已设置，则不必再重复拼接基础路径
  ...
  if (!cache$1.modules[item]) {
    ...
    node.src = url + function () {
      var version = config$3.version === true ? config$3.v || new Date().getTime() : config$3.version || '';
...
Class$g.prototype.getStyle = function (node, name) {
  ...
  return style.getPropertyValue ? style.getPropertyValue(name) : style.getAttribute(name.replace(/-(\w)/g, function (_, c) {
...
Class$g.prototype.link = function (href, callback, id) {
  ...
  if (typeof href === 'object') {
    ...
    return that.each(href, function (index, value) {
      that.link(value, index === href.length - 1 && callback, isArr && id[index]);
    });
  }
  ...
  if (!link) {
    ...
    link.href = href + (config$3.debug ? '?v=' + new Date().getTime() : '');
    link.rel = 'stylesheet';
    link.id = id;
...
Class$g.prototype.addcss = function (modName, callback, id) {
  return layui.link(config$3.dir + 'css/' + modName, callback, id);
};
...
Class$g.prototype.factory = function (modName) {
  if (layui[modName]) {
    return typeof config$3.callback[modName] === 'function' ? config$3.callback[modName] : null;
  }
};
...
Class$g.prototype.url = function (href) {
  ...
  var data = {
    ...
    search: function () {
      var obj = {};
      var search = (href ? function () {
        var str = (href.match(/\?.+/) || [])[0] || '';
        return str.replace(/#.+/, '');
      }() : location$1.search).replace(/^\?+/, '').split('&'); // 去除 ?，按 & 分割参数
    ...
    hash: that.router(function () {
      return href ? (href.match(/#.+/) || [])[0] || '/' : location$1.hash;
    }())
  };
  return data;
};
...
var Class$f = function (selector) {
  ...
  }() : (this.selector = selector, document$1.querySelectorAll(selector || null));
...
lay.onClickOutside = function (target, handler, options) {
  ...
  var cleanup = [bindEventListener(scopeTarget, eventType, listener, lay.passiveSupported ? {
    ...
    setTimeout(function () {
      if (document$1.activeElement && document$1.activeElement.tagName === 'IFRAME' && target.contains && !target.contains(document$1.activeElement)) {
        handler(event);
      }
    }, 0);
  })];
    ...
    disconnect: function () {
      for (var key in handlerCache) {
        if (lay.hasOwn(handlerCache, key)) {
          ...
          var elem = document$1.querySelector('[' + ATTR_NAME + '="' + key + '"]');
          if (elem) {
...
// 构造器
var Class$e = function (template, options) {
  var that = this;
 ...
 */
Class$e.prototype.render = function (template, data) {
  var that = this;
  var options = that.config;

  // 获得模板渲染函数
  var compiler = template ? that.compile(template) : that.compilerCache || that.compile(options.template);
 ...
 */
Class$e.prototype.compile = function (template) {
  var that = this;
  var options = that.config;
  ...
  var tagRegex = function (cores, sides) {
    var arr = ['(?:' + openDelimiter + (cores[0] || '') + '\\s*)',
    // 界定符前置
    '(' + (cores[1] || '[\\s\\S]') + '*?)',
    // 标签主体
  ...
  var parse = that.parse = function (tpl) {
    ...
    var output = function (str, delimiter, body) {
      ...
      return body ? strConcatenation('__laytpl__+=' + _escape + '(' + body + ');'
  ...
  var createBuilder = that.createBuilder = function (template, builder) {
    builder = builder || ['function(d){', '"use strict";', 'var __laytpl__="",' + function () {
      // 内部变量
      // 内部方法
      var arr = [];
      ...
      return arr.join(',');
    }() + ';', '__laytpl__="' + parse(template) + '";', 'return __laytpl__;',
 ...
 * @returns {string}
 */
Class$e.prototype.extractErrorContext = function (template, data) {
  var that = this;
  ...
  var errorContext = function (errLineNum) {
    ...
    for (; start <= end; start++) {
      arr.push((start == errLineNum ? '? ' : '  ') + (start + 1 + '| ') + templateArr[start]);
    }
    return arr.join('\n') + '\n';
  };
  try {
...
var laytpl = function (template, options) {
  var inst = new Class$e(template, options);
...
var resolveValue = memoize(function (path, obj, defaultValue) {
  ...
  if (layui.cache.debug) {
    ...
    if (isNotFound) {
      hint$5.errorOnce("Not found '" + path + "' key in '" + locale + "' locale messages.", 'warn');
    }
    if (isFallback) {
      hint$5.errorOnce("Fallback to default message for key: '" + path + "'", 'warn');
    }
  }
...
i18n.translation = function (keypath, parameters, options) {
  var locale = options && options.locale || config$1.locale;
  var i18nMessages = config$1.messages[locale];
  ...
  if (!i18nMessages && !hasDefault) {
    hint$5.errorOnce("Locale '" + locale + "' not found. Please add i18n messages for this locale first.", 'warn');
  }
...
function component$e(settings) {
  ...
  Class.prototype.init = function (rerender, type) {
    ...
    options.id = lay.hasOwn(options, 'id') ? options.id : elem.attr('id') || that.index;
...
// 判断传入的容器类型
Class$d.prototype.type = function () {
...
// 分页视图
Class$d.prototype.view = function () {
  var that = this;
  var config = that.config;

  // 连续页码个数
  var groups = config.groups = 'groups' in config ? Number(config.groups) || 0 : 5;
  ...
  var views = {
    // 上一页
    prev: function () {
      return config.prev ? '<a class="layui-laypage-prev' + (config.curr == 1 ? ' ' + DISABLED$2 : '') + '" data-page="' + (config.curr - 1) + '">' + config.prev + '</a>' : '';
    }(),
    // 页码
    page: function () {
      var pager = [];
    ...
    // 下一页
    next: function () {
      return config.next ? '<a class="layui-laypage-next' + (config.curr == config.pages ? ' ' + DISABLED$2 : '') + '" data-page="' + (config.curr + 1) + '">' + config.next + '</a>' : '';
    }(),
    // 数据总数
    count: function () {
      var countText = typeof config.countText === 'object' ? config.countText[0] + config.count + config.countText[1] : i18n.$t('laypage.total', {
        total: config.count
      });
    ...
    limit: function () {
      ...
      var template = function (item) {
        ...
        return typeof config.limitTemplet === 'function' ? config.limitTemplet(item) || def : def;
      };

      // 条目选项列表
      layui.each(config.limits, function (index, item) {
        elemArr.push('<option value="' + item + '"' + (item === config.limit ? ' selected' : '') + '>' + template(item) + '</option>');
      });
      return elemArr.join('') + '</select></span>';
    }(),
    // 刷新当前页
    refresh: ['<a data-page="' + config.curr + '" class="layui-laypage-refresh">', '<i class="layui-icon layui-icon-refresh"></i>', '</a>'].join(''),
    // 跳页区域
    skip: function () {
      var skipText = typeof config.skipText === 'object' ? config.skipText : [i18n.$t('laypage.goto'), i18n.$t('laypage.page'), i18n.$t('laypage.confirm')];
...
// 跳页的回调
Class$d.prototype.jump = function (elem, isskip) {
  if (!elem) return;
  var that = this;
  var config = that.config;
...
// 输入页数字控制
Class$d.prototype.skip = function (elem) {
  if (!elem) return;
  var that = this;
  var input = elem[tag]('input')[0];
  if (!input) return;
...
// 渲染分页
Class$d.prototype.render = function (load) {
  var that = this;
  var config = that.config;
  var type = that.type();
  var view = that.view();
...
// 模块名
var MOD_NAME$4 = 'laydate';
var MOD_ID$3 = 'lay-' + MOD_NAME$4 + '-id'; // 已渲染过的索引标记名
...
function addSpaceBetweenChars(str) {
  ...
  for (var i = 0; i < str.length - 1; i++) {
    ...
    // 在数字和非数字（非空格）之间添加空格
    if (isCharDigit && !isNextCharDigit && nextChar !== ' ' ||
    // 数字 → 非数字（非空格）
...
// 字符常量
var ELEM$2 = '.layui-laydate';
var THIS$1 = 'layui-this';
var DISABLED$1 = 'laydate-disabled';
...
var ELEM_MAIN$2 = 'layui-laydate-main';
...
var Class$c = function (options) {
  ...
  if (elem[0] && elem.attr(MOD_ID$3)) {
    var newThat = thisModule$2.getThis(elem.attr(MOD_ID$3));
    if (!newThat) return;
...
// 将日期格式字符转换为数组
thisModule$2.formatArr = function (format) {
...
// 是否闰年
Class$c.isLeapYear = function (year) {
  return year % 4 === 0 && year % 100 !== 0 || year % 400 === 0;
};

// 默认配置
Class$c.prototype.config = {
  type: 'date',
  // 控件类型，支持：year/month/date/time/datetime
  range: false,
  // 是否开启范围选择，即双控件
  format: 'yyyy-MM-dd',
  // 默认日期格式
  ...
  // 日期时间改变后的回调
  autoConfirm: true,
  // 是否自动确认（日期|年份|月份选择器非range下是否自动确认）
  shade: 0
};
Class$c.prototype.getI18nMessages = function () {
  var that = this;
  var options = that.config;
  ...
  return {
    ...
    formatErrorPrompt: function (format) {
      return i18n.$t('laydate.formatErrorPrompt', {
        format: format
      }, {
        locale: locale,
...
// 仅简体中文生效，不做国际化
Class$c.prototype.markerOfChineseFestivals = {
  '0-1-1': '元旦',
  '0-2-14': '情人',
  '0-3-8': '妇女',
  '0-3-12': '植树',
...
// 重载实例
Class$c.prototype.reload = function (options) {
  var that = this;
  that.config = lay.extend({}, that.config, options);
  that.init();
};
...
//初始准备
Class$c.prototype.init = function () {
  var that = this,
    options = that.config,
  ...
  that.autoCalendarModel = function () {
    ...
    that.rangeLinked = options.range && (options.type === 'date' || options.type === 'datetime') && (!that.startDate || !that.endDate || that.startDate && that.endDate && that.startDate.year === that.endDate.year && that.startDate.month === that.endDate.month);
  ...
  //将日期格式转化成数组
  that.format = thisModule$2.formatArr(options.format);
  ...
  lay.each(['min', 'max'], function (i, item) {
    ...
    if (typeof options[item] === 'number') {
      ...
      var day = options[item],
        ...
        STAMP = 86400000,
        //代表一天的毫秒数
        thisDate = new Date(day ? day < STAMP ? time + day * STAMP : day //如果数字小于一天的毫秒数，则数字为天数，否则为毫秒数
        : time);
...
//控件主体渲染
Class$c.prototype.render = function () {
  var that = this,
    options = that.config,
    ...
    elem = that.elem = lay.elem('div', {
      id: that.elemID,
      class: ['layui-laydate', options.range ? ' layui-laydate-range' : '', that.rangeLinked ? ' layui-laydate-linkage' : '', isStatic ? ' ' + ELEM_STATIC : '', options.fullPanel ? ' laydate-theme-fullpanel' : '',
      // 全面版
  ...
  lay.each(new Array(2), function (i) {
    ...
    elemMain[i] = lay.elem('div', {
      class: ELEM_MAIN$2 + ' laydate-main-list-' + i
    });
  ...
  if (options.shortcuts) {
    ...
    lay(divShortcut).html(function () {
      ...
      var timeBtn = lay(that.footer).find('.' + ELEM_TIME_BTN).removeClass(DISABLED$1);
      timeBtn && timeBtn.attr('lay-type') === 'date' && timeBtn[0].click();
      that.done(null, 'change');
      lay(this).addClass(THIS$1);
  ...
  //移除上一个控件
  that.remove(Class$c.thisElemDate);
  ...
  Class$c.thisElemDate = that.elemID;
...
//控件移除
Class$c.prototype.remove = function (prev) {
  var that = this,
    options = that.config,
...
//定位算法
Class$c.prototype.position = function () {
  var that = this,
    options = that.config;
...
// 提示
Class$c.prototype.hint = function (opts) {
  var that = this;
  that.config;
...
//获取递增/减后的年月
Class$c.prototype.getAsYM = function (Y, M, type) {
  type ? M-- : M++;
  if (M < 0) {
    M = 11;
    Y--;
...
//系统日期
Class$c.prototype.systemDate = function (newDate) {
...
//日期校验
Class$c.prototype.checkDate = function (fn) {
  var that = this,
    options = that.config,
    lang = that.i18nMessages,
  ...
  if (typeof value === 'string' && value) {
    if (that.EXP_IF.test(value)) {
      ...
      //格式不合法
      that.hint(lang.formatErrorPrompt(options.range ? options.format + ' ' + that.rangeStr + ' ' + options.format : options.format) + lang.autoResetPrompt);
      error = true;
    }
 ...
 */
Class$c.prototype.markRender = function (tdElem, YMD, markers) {
  var markText;
  if (typeof markers === 'object') {
    lay.each(markers || {}, function (key, title) {
      var keys = key.split('-');
      if ((keys[0] == YMD[0] || keys[0] == 0) && (
      //每年的每月
 ...
 * @returns Class
 */
Class$c.prototype.mark = function (td, YMD) {
  var that = this;
  var options = that.config;
 ...
 */
Class$c.prototype.holidaysRender = function (tdElem, YMD, markers) {
  ...
  var insertHtml = function (el, type, text) {
    el.find('div').html(['<span', ' class="laydate-day-holidays"', ' type="' + type + '"', '>', text, '</span>'].join(''));
  };
 ...
 * @returns Class
 */
Class$c.prototype.holidays = function (td, YMD) {
  var that = this;
  var options = that.config;
  var render = function (markers) {
 ...
 * @returns
 */
Class$c.prototype.cellRender = function (el, dateObj, panelMode) {
  var that = this;
  var options = that.config;
  if (typeof options.cellRender === 'function') {
    ...
    options.cellRender(dateObj, render, {
      originElem: el,
      type: panelMode
    });
  }
 ...
 * @param {Date} date
 */
Class$c.prototype.startOfYear = function (date) {
 ...
 * @param {Date} date
 */
Class$c.prototype.endOfYear = function (date) {
  var newDate = new Date(date);
 ...
 * @param {Date} date
 */
Class$c.prototype.startOfMonth = function (date) {
  var newDate = new Date(date);
  newDate.setDate(1);
  newDate.setHours(0, 0, 0, 0);
  return newDate;
 ...
 * @param {Date} date
 */
Class$c.prototype.endOfMonth = function (date) {
  var newDate = new Date(date);
 ...
 */
Class$c.prototype.addDays = function (date, amount) {
  var newDate = new Date(date);
 ...
 */
Class$c.prototype.isDisabledYearOrMonth = function (date, type, position) {
  var that = this;
  var options = that.config;
 ...
 * @returns {boolean}
 */
Class$c.prototype.isDisabledDate = function (date, opts) {
  opts = opts || {};
  var that = this;
  var options = that.config;
  ...
  return opts.type === 'year' || opts.type === 'month' ? that.isDisabledYearOrMonth(normalizedDate, opts.type, position) : options.disabledDate.call(options, normalizedDate, position);
};
 ...
 * @returns {boolean}
 */
Class$c.prototype.isDisabledTime = function (date, opts) {
  opts = opts || {};
  var that = this;
  var options = that.config;
  var position = options.range ? opts.rangeType === 0 ? 'start' : 'end' : 'start';
 ...
 * @returns
 */
Class$c.prototype.isDisabledDateTime = function (timestamp, opts) {
  opts = opts || {};
  var that = this;
  that.config;
 ...
 *
 */
Class$c.prototype.limit = function (opts) {
  opts = opts || {};
  var that = this;
  var options = that.config;
  var timestamp = {};
  ...
  opts.elem && opts.elem[isOut ? 'addClass' : 'removeClass'](DISABLED$1);
  return isOut;
};

//当前日期对象
Class$c.prototype.thisDateTime = function (index) {
  var that = this,
    options = that.config;
...
//日历表
Class$c.prototype.calendar = function (value, index, type) {
  index = index ? 1 : 0;
  var that = this,
    options = that.config,
  ...
  lay.each(tds, function (index_, item) {
    ...
    if (index_ < startWeek) {
      ...
      st = index_ - startWeek;
      if (!that.rangeLinked) {
        st + 1 === dateTime.date && item.addClass(THIS$1);
      }
    } else {
  ...
  if (isAlone) {
    ...
    if (options.range) {
      if (value || type !== 'init') {
        // 判断是否需要显示年月时间列表
        that.listYM = [[(that.startDate || options.dateTime).year, (that.startDate || options.dateTime).month + 1], [that.endDate.year, that.endDate.month + 1]];
        that.list(options.type, 0).list(options.type, 1);

        //同步按钮可点状态
        options.type === 'time' ? that.setBtnStatus(true, lay.extend({}, that.systemDate(), that.startTime), lay.extend({}, that.systemDate(), that.endTime)) : that.setBtnStatus(true);
      }
    } else {
  ...
  // 重置快捷栏选中状态
  lay(that.shortcut).find('li.' + THIS$1).removeClass(THIS$1);
...
//生成年月时分秒列表
Class$c.prototype.list = function (type, index) {
  var that = this,
    options = that.config,
  ...
  if (type === 'year') {
    ...
    lay.each(new Array(15), function (i) {
      ...
      yearNum == listYM[0] && lay(li).addClass(THIS$1);
    ...
    lay(elemYM[!isMonthBeforeYear ? 0 : 1]).attr('lay-ym', yearNum - 8 + '-' + listYM[1]).html(startY + text + ' - ' + (yearNum - 1 + text));
  }
  ...
  //生成月列表
    ...
    lay.each(new Array(12), function (i) {
      ...
      i + 1 == listYM[1] && lay(li).addClass(THIS$1);
    ...
    lay(elemYM[!isMonthBeforeYear ? 0 : 1]).attr('lay-ym', listYM[0] + '-' + listYM[1]).html(listYM[0] + text);
  }
  ...
  //生成时间列表
      ...
      lay(ul).find('ol').each(function (i, ol) {
        lay(ol).find('li').each(function (ii, li) {
          that.limit({
            ...
            disabledType: 'time',
            // 时间面板，分别检测时分秒列表是否禁用
            time: [['hours'], ['hours', 'minutes'], ['hours', 'minutes', 'seconds']][i]
          });
        });
      });
      if (!options.range) {
    ...
    lay.each([24, 60, 60], function (i, item) {
      ...
      lay.each(new Array(item), function (ii) {
        childUL.push('<li' + (that[startEnd][hms[i]] === ii ? ' class="' + THIS$1 + '"' : '') + '>' + lay.digit(ii, 2) + '</li>');
      });
  ...
  if (type === 'year' || type === 'month') {
    ...
    lay(ul).find('li').on('click', function () {
      ...
      if (lay(this).hasClass(DISABLED$1)) return;
      if (that.rangeLinked) {
        lay.extend(dateTime, {
      ...
      if (isYearOrMonth || isChangeMonth) {
        lay(ul).find('.' + THIS$1).removeClass(THIS$1);
        lay(this).addClass(THIS$1);
      ...
      lay(that.footer).find('.' + ELEM_TIME_BTN).removeClass(DISABLED$1);
    });
  } else {
    //时间选择面板 - 选择事件
        ...
        lay(ul).find('ol').each(function (i) {
          ...
          if (ol.scrollTop <= 0) {
            li.each(function (ii, item) {
              if (!lay(this).hasClass(DISABLED$1)) {
    ...
    olElem.each(function (i) {
      ...
      lay(ol).find('li').on('click', function () {
        ...
        lay(ol).find('.' + THIS$1).removeClass(THIS$1);
        lay(this).addClass(THIS$1);
        setTimeStatus();
        scroll();
...
//记录列表切换后的年月
Class$c.prototype.listYM = [];

//关闭列表
Class$c.prototype.closeList = function () {
  var that = this;
    that.config;
...
//检测结束日期是否超出开始日期
Class$c.prototype.setBtnStatus = function (tips, start, end) {
  var that = this,
    options = that.config,
    lang = that.i18nMessages,
    isOut,
  ...
  if (options.range) {
    ...
    that.limit({
      ...
      time: timeParams,
      rangeType: 1
    }) ? elemBtn.addClass(DISABLED$1) : elemBtn[isOut ? 'addClass' : 'removeClass'](DISABLED$1);
...
// 转义为规定格式的日期字符
Class$c.prototype.parse = function (state, date) {
  var that = this;
  var options = that.config;
  var startDate = that.rangeLinked ? that.startDate : options.dateTime;
  var dateTime = date || (state == 'end' ? lay.extend({}, that.endDate, that.endTime) : options.range ? lay.extend({}, startDate || options.dateTime, that.startTime) : options.dateTime);
 ...
 */
Class$c.prototype.formatToDisplay = function (elem, displayValueCallback) {
  var that = this;
  var props = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value');
  Object.defineProperty(elem, 'value', lay.extend({}, props, {
    get: function () {
...
//赋值
Class$c.prototype.setValue = function (value) {
  var that = this,
    options = that.config,
    elem = options.elem[0];
...
//预览
Class$c.prototype.preview = function () {
  var that = this,
    options = that.config;
...
Class$c.prototype.renderAdditional = function () {
  var that = this;
  var options = that.config;
...
// 标记范围内的日期
Class$c.prototype.stampRange = function () {
  var that = this,
    options = that.config,
    startTime = that.rangeLinked ? that.startDate : options.dateTime,
    endTime,
    tds = lay(that.elem).find('td');
  if (options.range && !that.endState) lay(that.footer).find(ELEM_CONFIRM).addClass(DISABLED$1);
  ...
  lay.each(tds, function (i, item) {
    ...
    if (options.rangeLinked && !that.startDate) {
      if (thisTime === that.newDate(that.systemDate()).getTime()) {
        lay(item).addClass(lay(item).hasClass(ELEM_PREV) || lay(item).hasClass(ELEM_NEXT) ? '' : ELEM_DAY_NOW);
      }
    }
    ...
    lay(item).removeClass(ELEM_SELECTED + ' ' + THIS$1);
    if (thisTime === startTime || thisTime === endTime) {
      (that.rangeLinked || !that.rangeLinked && (i < 42 ? thisTime === startTime : thisTime === endTime)) && lay(item).addClass(lay(item).hasClass(ELEM_PREV) || lay(item).hasClass(ELEM_NEXT) ? ELEM_SELECTED : THIS$1);
    }
    if (thisTime > startTime && thisTime < endTime) {
...
// 执行 done/change 回调
Class$c.prototype.done = function (param, type) {
  var that = this;
  var options = that.config;
  var start = lay.extend({}, lay.extend(that.rangeLinked ? that.startDate : options.dateTime, that.startTime));
 ...
 * @returns
 */
Class$c.prototype.checkPanelDate = function (datetime, index) {
  var that = this;
  var options = that.config;
  // 年月范围选择不需要处理
  ...
  // 3. 右侧面板同理。
  function () {
    return startPanel && month > panelMonth || !startPanel && month < panelMonth ? 1 - index : index;
  }() :
...
//选择日期
Class$c.prototype.choose = function (td, index) {
  if (td.hasClass(DISABLED$1)) return;
  var that = this,
    options = that.config,
...
//底部按钮
Class$c.prototype.tool = function (btn, type) {
  var that = this,
    options = that.config,
    lang = that.i18nMessages,
    dateTime = options.dateTime,
    ...
    active = {
      //选择时间
      datetime: function () {
        if (lay(btn).hasClass(DISABLED$1)) return;
        that.list('time', 0);
      ...
      clear: function () {
        ...
        options.range && (delete options.dateTime, delete that.endDate, delete that.startTime, delete that.endTime);
        that.setValue('');
      ...
      now: function () {
        ...
        if (lay(btn).hasClass(DISABLED$1)) {
      ...
      confirm: function () {
        if (options.range) {
          if (lay(btn).hasClass(DISABLED$1)) {
            var isTimeout = options.type === 'time' ? that.startTime && that.endTime && that.newDate(that.startTime) > that.newDate(that.endTime) : that.startDate && that.endDate && that.newDate(lay.extend({}, that.startDate, that.startTime || {})) > that.newDate(lay.extend({}, that.endDate, that.endTime || {}));
            return that.hint(isTimeout ? lang.rangeOrderPrompt : lang.invalidDatePrompt);
          }
        } else {
          if (lay(btn).hasClass(DISABLED$1)) return that.hint(lang.invalidDatePrompt);
        }
        that.setValue(that.parse());
...
//统一切换处理
Class$c.prototype.change = function (index) {
  var that = this,
    options = that.config,
...
//日期切换事件
Class$c.prototype.changeEvent = function () {
  var that = this;
    that.config;
  ...
  lay.each(that.elemHeader, function (i, header) {
    ...
    lay(header[2]).find('span').on('click', function () {
      ...
      that.list(layType, i);
      lay(that.footer).find('.' + ELEM_TIME_BTN).addClass(DISABLED$1);
    });
...
Class$c.prototype.events = function () {
  ...
  that.unbind = function () {
    ...
    delete thisModule$2.that[options.id];
  };
};
...
lay(document).on('mousedown', function (e) {
  ...
  var that = thisModule$2.getThis(laydate.thisId);
  if (!that) return;
  var options = that.config;
  if (e.target === options.elem[0] || e.target === options.eventElem[0] || e.target === lay(options.closeStop)[0] || options.elem[0] && options.elem[0].contains(e.target)) return;
  that.remove();
  ...
  if (e.keyCode === 13) {
    if (lay('#' + that.elemID)[0] && that.elemID === Class$c.thisElemDate) {
      e.preventDefault();
...
// 获取当前实例对象
thisModule$2.getThis = function (id) {
  var that = thisModule$2.that[id];
  if (!that) {
    layui.hint().error(id ? MOD_NAME$4 + " instance with ID '" + id + "' not found" : 'ID argument required');
  }
  return that;
};
...
laydate.close = function (id) {
  var that = thisModule$2.getThis(id || laydate.thisId);
  if (!that) return;
  return that.remove();
};
...
laydate.parse = function (dateTime, format, one) {
  ...
  if (typeof format === 'string') {
    format = thisModule$2.formatArr(format);
  }
...
// 容器
Class$b.pt.vessel = function (conType, callback) {
  var that = this,
    times = that.index,
    config = that.config;
  ...
  // 表情或图标
  function () {
    var face = ['layui-icon-tips', 'layui-icon-success', 'layui-icon-error', 'layui-icon-question', 'layui-icon-lock', 'layui-icon-face-cry', 'layui-icon-face-smile'];
    var additFaceClass;
  ...
  if (config.id && $('.' + doms[0]).find('#' + config.id)[0]) {
    return function () {
      ...
      if (maxminStatus === 'min') {
        ...
        setTimeout(function () {
          elemShade.css({
            opacity: elemShade.data(SHADE_KEY)
          });
        }, 10);
      }
    }();
...
// 自适应
Class$b.pt.auto = function (index) {
  var that = this,
    config = that.config,
    layero = $('#' + doms[0] + index);
...
Class$b.pt.btnLoading = function (btnElem, isLoading) {
  if (isLoading) {
...
Class$b.pt.callback = function () {
  ...
  layero.find('.' + doms[6]).children('a').on('click', function () {
    ...
    if (config.btnAsync) {
      ...
      if (btnCallback) {
        ready.promiseLikeResolve(btnCallback.call(config, that.index, layero, that)).then(function (result) {
          if (result !== false) {
            layer.close(that.index);
          }
        }, function (reason) {
...
ready.record = function (layero) {
  ...
  var area = [layero[0].style.width || ready.getStyle(layero[0], 'width'), layero[0].style.height || ready.getStyle(layero[0], 'height'), layero.position().top, layero.position().left + parseFloat(layero.css('margin-left'))];
...
ready.restScrollbar = function (index) {
  ...
  var targetEl = $('.' + doms[0]).filter(function () {
    var layero = $(this);
    var options = layero.data('config') || {};
    return options.scrollbar === false && layero.data('maxminStatus') !== 'min' && layero.attr('times') !== String(index);
  });
...
ready.updatePosition = function (layero, config) {
  ...
  if (!config.fixed) {
    coords.offsetTop = /%$/.test(coords.offsetTop) ? win.height() * parseFloat(coords.offsetTop) / 100 : parseFloat(coords.offsetTop);
    coords.offsetLeft = /%$/.test(coords.offsetLeft) ? win.width() * parseFloat(coords.offsetLeft) / 100 : parseFloat(coords.offsetLeft);
...
layer.iframeAuto = function (index) {
  ...
  layero.css({
    height: iframeHeight + titleHeight + btnHeight
  });
...
layer.close = function (index, callback) {
  ...
  if (!hideOnClose && typeof ready.beforeEnd[index] === 'function') {
    ready.promiseLikeResolve(ready.beforeEnd[index]()).then(function (result) {
      if (result !== false) {
...
layer.prompt = function (options, yes) {
  ...
  var prompt;
  var content = options.formType == 'textarea' ? '<textarea class="layui-layer-input"' + style + placeholder + '></textarea>' : function () {
  ...
  return layer.open($.extend({
    ...
    yes: function (index) {
      ...
      if (value.length > (options.maxlength || 500)) {
        layer.tips(i18n.$t('layer.prompt.InputLengthPrompt', {
  ...
  loadImage(data[start].src, function (img) {
    ...
    dict.index = layer.open($.extend({
      ...
      content: '<div class="layer-layer-photos-main">' + '<img src="' + data[start].src + '" alt="' + alt + '" layer-pid="' + (data[start].pid || '') + '">' + function () {
        ...
        // 头部工具栏
        if (options.toolbar) {
          arr.push(['<div class="layui-layer-photos-toolbar layui-layer-photos-header">', '<span toolbar-event="rotate" data-option="90" title="' + i18n.$t('layer.photos.tools.rotate') + '"><i class="layui-icon layui-icon-refresh"></i></span>', '<span toolbar-event="scalex" title="' + i18n.$t('layer.photos.tools.scaleX') + '"><i class="layui-icon layui-icon-slider"></i></span>', '<span toolbar-event="zoom" data-option="0.1" title="' + i18n.$t('layer.photos.tools.zoomIn') + '"><i class="layui-icon layui-icon-add-circle"></i></span>', '<span toolbar-event="zoom" data-option="-0.1" title="' + i18n.$t('layer.photos.tools.zoomOut') + '"><i class="layui-icon layui-icon-reduce-circle"></i></span>', '<span toolbar-event="reset" title="' + i18n.$t('layer.photos.tools.reset') + '"><i class="layui-icon layui-icon-refresh-1"></i></span>', '<span toolbar-event="close" title="' + i18n.$t('layer.photos.tools.close') + '"><i class="layui-icon layui-icon-close"></i></span>', '</div>'].join(''));
        }

        // 底部栏
        if (options.footer) {
          arr.push(['<div class="layui-layer-photos-toolbar layui-layer-photos-footer">', '<h3>' + alt + '</h3>', '<em>' + dict.imgIndex + ' / ' + data.length + '</em>', '<a href="' + data[start].src + '" target="_blank">' + i18n.$t('layer.photos.viewPicture') + '</a>', '</div>'].join(''));
        }
        arr.push('</div>');
        return arr.join('');
      }() + '</div>',
...
var util = {
  ...
  fixbar: function (options) {
    ...
    // 滚动条所在元素对象
    var $scroll = options.scroll ? $(options.scroll) : $(options.target === 'body' ? $doc : $target);
  ...
  toDateString: function (time, format, options) {
    ...
    if (!date.getDate()) return hint$4.error('Invalid millisecond for "util.toDateString(millisecond)"'), '';
...
var clickOrMousedown$1 = device$4.mobile ? 'touchstart' : 'mousedown';

// 模块名
var MOD_NAME$3 = 'dropdown';
var MOD_INDEX$1 = 'layui_' + MOD_NAME$3 + '_index'; // 模块索引名
var MOD_INDEX_OPENED = MOD_INDEX$1 + '_opened';
var MOD_ID$2 = 'lay-' + MOD_NAME$3 + '-id';
var resizeObserver$2 = lay.createSharedResizeObserver(MOD_NAME$3);
...
// 重载实例
Class$a.prototype.reload = function (options, type) {
  var that = this;
  that.config = $.extend({}, that.config, options);
  that.init(true, type);
};
...
// 初始化准备
Class$a.prototype.init = function (rerender, type) {
  var that = this;
  var options = that.config;
  ...
  if (!rerender && elem.attr(MOD_ID$2)) {
    var newThat = thisModule$1.getThis(elem.attr(MOD_ID$2));
    if (!newThat) return;
    return newThat.reload(options, type);
  }
  options.elem = $(options.elem);
  ...
  // 初始化 id 属性 - 优先取 options > 元素 id > 自增索引
  options.id = 'id' in options ? options.id : elem.attr('id') || that.index;
  thisModule$1.that[options.id] = that; // 记录当前实例对象
...
// 渲染
Class$a.prototype.render = function (type) {
  var that = this;
  var options = that.config;
  ...
  // 主模板
  var TPL_MAIN = ['<div class="layui-dropdown layui-border-box layui-panel layui-anim layui-anim-downbit" ' + MOD_ID$2 + '="' + options.id + '">', '</div>'].join('');

  // 重载或插入面板内容
  var mainElem;
  var content = options.content || getDefaultView();
  var mainElemExisted = thisModule$1.findMainElem(options.id);
  if (type === 'reloadData' && mainElemExisted.length) {
    ...
    var shadeElem = $(shade);
    // 处理移动端点击穿透问题
    if (clickOrMousedown$1 === 'touchstart') {
      shadeElem.on(clickOrMousedown$1, function (e) {
        e.preventDefault();
      });
    }
  ...
  mainElem.find(STR_GROUP_TITLE).on('click', function (e) {
    ...
    if (data.type === 'group' && options.isAllowSpread) {
      thisModule$1.spread(elemGroup, options.accordion);
    }
  });
  that.onClickOutside();
...
// 位置定位
Class$a.prototype.position = function (obj) {
  var that = this;
  var options = that.config;
...
// 移除面板
Class$a.prototype.remove = function (id) {
  id = id || this.config.id;
  var that = thisModule$1.getThis(id); // 根据 id 查找对应的实例
  if (!that) return;
  var options = that.config;
  var mainElem = thisModule$1.findMainElem(id);
...
Class$a.prototype.normalizedDelay = function () {
  var that = this;
  var options = that.config;
...
// 延迟移除面板
Class$a.prototype.delayRemove = function () {
  var that = this;
  that.config;
  clearTimeout(that.timer);
 ...
 * 点击面板外部时的事件
 */
Class$a.prototype.onClickOutside = function () {
  var that = this;
  var options = that.config;
 ...
 * 窗口大小变化时自动更新位置
 */
Class$a.prototype.autoUpdatePosition = function () {
  var that = this;
  var options = that.config;
  that.stopResizeEvent();
  ...
  var shouldObserveResize = resizeObserver$2 && options.trigger !== 'contextmenu';
  var triggerEl = options.elem[0];
  var contentEl = that.mainElem[0];
  if (shouldObserveResize) {
    resizeObserver$2.observe(triggerEl, $.proxy(that.position, that));
    resizeObserver$2.observe(contentEl, $.proxy(that.position, that));
  }
  that.stopResizeEvent = function () {
    $(window).off('resize.lay_dropdown_resize', windowResizeHandler);
    if (shouldObserveResize) {
      resizeObserver$2.unobserve(triggerEl);
      resizeObserver$2.unobserve(contentEl);
    }
    that.stopResizeEvent = $.noop;
  };
};
...
// 获取当前实例对象
thisModule$1.getThis = function (id) {
  if (id === undefined) {
    throw new Error('ID argument required');
  }
  return thisModule$1.that[id];
};

// 根据 id 从页面查找组件主面板元素
thisModule$1.findMainElem = function (id) {
  return $('.' + STR_ELEM + '[' + MOD_ID$2 + '="' + id + '"]');
};

// 设置菜单组展开和收缩状态
thisModule$1.spread = function (othis, isAccordion) {
...
(function () {
  ...
  _DOC.on('click', ELEM_LI, function (e) {
    ...
    if (!isChild) {
      ...
      // 触发事件
      layui.event.call(this, MOD_NAME$3, 'click(' + filter + ')', options);
    }
  });
  ...
  _DOC.on('click', ELEM_LI + STR_GROUP_TITLE, function (e) {
    ...
    if ('isAllowSpread' in options ? options.isAllowSpread : true) {
      thisModule$1.spread(elemGroup, isAccordion);
    }
  });
...
dropdown.close = function (id) {
  var that = thisModule$1.getThis(id);
  if (!that) return this;
  that.remove();
...
var component$d = component$e({
  ...
  config: {
    ...
    // 输入框，关闭
    range: false,
    // 范围选择，与输入框不能同时开启，默认关闭
    height: 200,
    // 配合 type:"vertical" 使用，默认200px
    disabled: false,
    // 滑块禁用，默认关闭
    theme: '#16baaa' // 主题颜色
  },
  ...
  render: function () {
    ...
    //滑块
    var temp = '<div class="layui-slider ' + (options.type === 'vertical' ? 'layui-slider-vertical' : '') + '">' + (options.tips ? '<div class="' + CONST$b.SLIDER_TIPS + '" ' + (options.tipsAlways ? '' : 'style="display:none;"') + '></div>' : '') + '<div class="layui-slider-bar" style="background:' + theme + '; ' + (options.type === 'vertical' ? 'height' : 'width') + ':' + scale + ';' + (options.type === 'vertical' ? 'bottom' : 'left') + ':' + (scaleFir || 0) + ';"></div><div class="layui-slider-wrap" style="' + (options.type === 'vertical' ? 'bottom' : 'left') + ':' + (scaleFir || scale) + ';">' + '<div class="layui-slider-wrap-btn" style="border: 2px solid ' + theme + ';"></div></div>' + (options.range ? '<div class="layui-slider-wrap" style="' + (options.type === 'vertical' ? 'bottom' : 'left') + ':' + scaleSec + ';"><div class="layui-slider-wrap-btn" style="border: 2px solid ' + theme + ';"></div></div>' : '') + '</div>';
    var othis = $(options.elem);
    var hasRender = othis.next('.' + CONST$b.ELEM_VIEW);
    //生成替代元素
    ...
    //把数据缓存到滑块上
    if (options.range) {
      that.elemTemp.find('.' + CONST$b.SLIDER_WRAP).eq(0).data('value', options.value[0]);
      that.elemTemp.find('.' + CONST$b.SLIDER_WRAP).eq(1).data('value', options.value[1]);
    } else {
      that.elemTemp.find('.' + CONST$b.SLIDER_WRAP).data('value', options.value);
    }
    ...
    if (options.input && !options.range) {
      ...
      othis.find('.' + CONST$b.SLIDER_INPUT_TXT).children('input').val(options.value);
      if (options.type === 'vertical') {
        elemInput.css({
          left: 0,
          top: -48
    ...
    if (!options.disabled) {
      ...
      that.elemTemp.addClass(CONST$b.CLASS_DISABLED);
      that.elemTemp.find('.' + CONST$b.SLIDER_WRAP_BTN).addClass(CONST$b.CLASS_DISABLED);
    }
    ...
    function setSliderTipsTxt(sliderWrapBtnElem) {
      ...
      that.elemTemp.find('.' + CONST$b.SLIDER_TIPS).html(tipsTxt);
    }
    ...
    function calcSliderTipsLeft(sliderWrapBtnElem) {
      ...
      var sliderWrap = that.elemTemp.find('.' + CONST$b.SLIDER_WRAP);
    ...
    function setSliderTipsLeft(left) {
      if (options.type === 'vertical') {
        that.elemTemp.find('.' + CONST$b.SLIDER_TIPS).css({
          bottom: left + '%',
          'margin-bottom': '20px',
    ...
    if (options.tips) {
      if (options.tipsAlways) {
        var sliderWrapBtnElem = that.elemTemp.find('.' + CONST$b.SLIDER_WRAP_BTN);
        ...
        //划过滑块显示数值
        var timer;
        that.elemTemp.find('.' + CONST$b.SLIDER_WRAP_BTN).on('mouseover', function () {
          ...
          clearTimeout(timer);
          if (!options.tipsAlways) {
            that.elemTemp.find('.' + CONST$b.SLIDER_TIPS).css('display', 'none');
          }
        });
      }
    }
...
var CONST$b = component$d.CONST;
var Class$9 = component$d.Class;

// 数值精度
Class$9.prototype.precision = function () {
  var that = this;
  var options = that.config;
  var precisions = $.map([options.min, options.max, options.step], function (v) {
...
//滑块滑动
Class$9.prototype.slide = function (setValue, value, i) {
  var that = this;
  var options = that.config;
  ...
  var sliderWrap = sliderAct.find('.' + CONST$b.SLIDER_WRAP);
  var sliderTxt = sliderAct.next('.' + CONST$b.SLIDER_INPUT);
  var inputValue = sliderTxt.children('.' + CONST$b.SLIDER_INPUT_TXT).children('input').val();
  ...
  var change = function (offsetValue, index, from) {
    ...
    if (options.type === 'vertical') {
      sliderAct.find('.' + CONST$b.SLIDER_TIPS).css({
        bottom: offsetValue + '%',
        'margin-bottom': '20px'
      });
      ...
      sliderAct.find('.' + CONST$b.SLIDER_TIPS).css('left', offsetValue + '%');
    }
    ...
    if (options.type === 'vertical') {
      sliderAct.find('.' + CONST$b.SLIDER_BAR).css({
        height: wrapWidth + '%',
        bottom: minLeft + '%'
      });
    } else {
    ...
    sliderTxt.children('.' + CONST$b.SLIDER_INPUT_TXT).children('input').val(inputValue);
    sliderWrap.eq(index).data('value', selfValue);
    sliderAct.find('.' + CONST$b.SLIDER_TIPS).html(options.setTips ? options.setTips(selfValue) : selfValue);

    //如果开启范围选择，则返回数组值
    if (options.range) {
      var arrValue = [sliderWrap.eq(0).data('value'), sliderWrap.eq(1).data('value')];
  ...
  var createMoveElem = function (sliderBtnElem, move, up) {
    var upCall = function () {
      ...
      if (lay.touchEventsSupported()) {
        sliderBtnElem[0].removeEventListener('touchmove', move, lay.passiveSupported ? {
          passive: false
        } : false);
    ...
    // 移动端
    if (lay.touchEventsSupported()) {
      sliderBtnElem[0].addEventListener('touchmove', move, lay.passiveSupported ? {
        passive: false
      } : false);
  ...
  //滑块滑动
  sliderAct.find('.' + CONST$b.SLIDER_WRAP_BTN).each(function (index) {
    var othis = $(this);
    othis.on('mousedown touchstart', function (e) {
      ...
      var move = function (e) {
        ...
        othis.addClass(CONST$b.ELEM_HOVER);
        sliderAct.find('.' + CONST$b.SLIDER_TIPS).show();
        e.preventDefault();
      };
      var up = function (delay) {
        othis.removeClass(CONST$b.ELEM_HOVER);
        if (!options.tipsAlways) {
          setTimeout(function () {
            sliderAct.find('.' + CONST$b.SLIDER_TIPS).hide();
          }, delay);
        }
      };
  ...
  sliderAct.on('click', function (e) {
    var main = $('.' + CONST$b.SLIDER_WRAP_BTN);
    var othis = $(this);
    if (!main.is(event.target) && main.has(event.target).length === 0 && main.length) {
      var index;
      var offset = options.type === 'vertical' ? sliderWidth() - e.clientY + othis.offset().top - $(window).scrollTop() : e.clientX - othis.offset().left - $(window).scrollLeft();
  ...
  //点击加减输入框
  sliderTxt.children('.' + CONST$b.SLIDER_INPUT_BTN).children('i').each(function (index) {
    $(this).on('click', function () {
      inputValue = sliderTxt.children('.' + CONST$b.SLIDER_INPUT_TXT).children('input').val();
      if (index == 1) {
        //减
        inputValue = inputValue - options.step < options.min ? options.min : Number(inputValue) - options.step;
      } else {
        inputValue = Number(inputValue) + options.step > options.max ? options.max : Number(inputValue) + options.step;
      }
  ...
  sliderTxt.children('.' + CONST$b.SLIDER_INPUT_TXT).children('input').on('keydown', function (e) {
    if (e.keyCode === 13) {
      e.preventDefault();
...
var clickOrMousedown = device$3.mobile ? 'click' : 'mousedown';

// 创建组件
var component$c = component$e({
  name: 'colorpicker',
  // 默认配置
  config: {
    color: '',
  ...
  beforeInit: function () {
    ...
    that.stopClickOutsideEvent = $.noop;
    that.stopResizeEvent = $.noop;
    CONST$a.PICKER_OPENED = CONST$a.MOD_ID + '-opened';
  },
  // 渲染之前
  ...
  render: function () {
    ...
    var elemColorBox = $(['<div class="layui-unselect layui-colorpicker">', '<span ' + (options.format == 'rgb' && options.alpha ? 'class="layui-colorpicker-trigger-bgcolor"' : '') + '>', '<span class="layui-colorpicker-trigger-span" ', 'lay-type="' + (options.format == 'rgb' ? options.alpha ? 'rgba' : 'torgb' : '') + '" ', 'style="' + function () {
      ...
      return bgstr;
    }() + '">', '<i class="layui-icon layui-colorpicker-trigger-i ' + (options.color ? CONST$a.ICON_PICKER_DOWN : CONST$a.ICON_PICKER_CLOSE) + '"></i>', '</span>', '</span>', '</div>'].join(''));
    ...
    // 获取背景色值
    that.color = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_SPAN)[0].style.background;
  }
});
...
var $win$2 = $(window);
$(document);
var CONST$a = component$c.CONST;

/**
 * 扩展组件原型方法
 */

var Class$8 = component$c.Class;

// 渲染颜色选择器
Class$8.prototype.renderPicker = function () {
  var that = this;
  var options = that.config;
  ...
  //透明度条块
  '<div class="layui-colorpicker-main-alpha ' + (options.alpha ? CONST$a.CLASS_SHOW : '') + '">', '<div class="layui-colorpicker-alpha-bgcolor">', '<div class="layui-colorpicker-alpha-slider"></div>', '</div>', '</div>',
  //预设颜色列表
  function () {
  ...
  options.elem.data(CONST$a.PICKER_OPENED, true); // 面板已打开的标记
...
// 颜色选择器移除
Class$8.prototype.removePicker = function (index) {
  var that = this;
  var options = that.config;
  ...
  if (elem[0]) {
    elem.remove();
    options.elem.removeData(CONST$a.PICKER_OPENED);
...
// 面板定位
Class$8.prototype.position = function () {
  var that = this;
  var options = that.config;
...
// 颜色选择器赋值
Class$8.prototype.val = function () {
  var that = this;
    that.config;
    var elemColorBox = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_SPAN),
    elemPickerInput = that.elemPicker.find('.' + CONST$a.PICKER_INPUT),
    e = elemColorBox[0],
  ...
  if (bgcolor) {
    ...
    if (type === 'torgb') {
      ...
      if ((bgcolor.match(/[0-9]{1,3}/g) || []).length === 3) {
        ...
        that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_SLIDER).css('left', 280);
      } else {
        elemPickerInput.find('input').val(bgcolor);
        var left = bgcolor.slice(bgcolor.lastIndexOf(',') + 1, bgcolor.length - 1) * 280;
        that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_SLIDER).css('left', left);
      }

      // 设置 span 背景色
      that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_BG)[0].style.background = 'linear-gradient(to right, rgba(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + ', 0), rgb(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + '))';
    } else {
    ...
    that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_BG)[0].style.background = '';
    that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_SLIDER).css('left', 280);
  }
};

// 颜色选择器滑动 / 点击
Class$8.prototype.side = function () {
  var that = this,
    options = that.config,
    span = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_SPAN),
    type = span.attr('lay-type'),
    side = that.elemPicker.find('.' + CONST$a.PICKER_SIDE),
    slider = that.elemPicker.find('.' + CONST$a.PICKER_SIDE_SLIDER),
    basis = that.elemPicker.find('.' + CONST$a.PICKER_BASIS),
    choose = that.elemPicker.find('.' + CONST$a.PICKER_BASIS_CUR),
    alphacolor = that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_BG),
    alphaslider = that.elemPicker.find('.' + CONST$a.PICKER_ALPHA_SLIDER),
    ...
    i = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_I),
    ...
    change = function (x, y, z, a) {
      ...
      var elemInput = that.elemPicker.find('.' + CONST$a.PICKER_INPUT).find('input');
      i.addClass(CONST$a.ICON_PICKER_DOWN).removeClass(CONST$a.ICON_PICKER_CLOSE);
      ...
      //回调更改的颜色
      options.change && options.change($.trim(that.elemPicker.find('.' + CONST$a.PICKER_INPUT).find('input').val()));
    },
    //拖拽元素
  ...
  side.on('mousedown', function (e) {
    var top = e.clientY - $(this).offset().top + $win$2.scrollTop();
    if (top < 0) top = 0;
  ...
  basis.on('mousedown', function (e) {
    var top = e.clientY - $(this).offset().top + $win$2.scrollTop(),
      left = e.clientX - $(this).offset().left + $win$2.scrollLeft();
    if (top < 0) top = 0;
    if (top > this.offsetHeight) top = this.offsetHeight;
    if (left < 0) left = 0;
...
// 颜色选择器hsb转换
Class$8.prototype.select = function (h, s, b, type) {
  var that = this;
  that.config;
  var hex = HSBToHEX({
    h: h,
  ...
  var left = s / 100 * 260;
  var basisElem = that.elemPicker.find('.' + CONST$a.PICKER_BASIS)[0];
  that.elemPicker.find('.' + CONST$a.PICKER_SIDE_SLIDER).css('top', sidetop); //滑块的top
  basisElem.style.background = '#' + hex; //颜色选择器的背景

  //选择器的top left
  that.elemPicker.find('.' + CONST$a.PICKER_BASIS_CUR).css({
...
Class$8.prototype.pickerEvents = function () {
  var that = this;
  var options = that.config;
  var elemColorBoxSpan = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_SPAN); //颜色盒子
  var elemPickerInput = that.elemPicker.find('.' + CONST$a.PICKER_INPUT + ' input'); //颜色选择器表单

  var pickerEvents = {
    // 清空
    clear: function (othis) {
      elemColorBoxSpan[0].style.background = '';
      that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_I).removeClass(CONST$a.ICON_PICKER_DOWN).addClass(CONST$a.ICON_PICKER_CLOSE);
      that.color = '';
    ...
    confirm: function (othis, change) {
      ...
      if (value.indexOf(',') > -1) {
        ...
        hsb = HEXToHSB(value);
        elemColorBoxSpan[0].style.background = colorValue = '#' + HSBToHEX(hsb);
        that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_I).removeClass(CONST$a.ICON_PICKER_CLOSE).addClass(CONST$a.ICON_PICKER_DOWN);
      }
      if (change === 'change') {
...
Class$8.prototype.events = function () {
  ...
  that.elemColorBox.on('click', function () {
    // 主面板是否已打开
    var opened = options.elem.data(CONST$a.PICKER_OPENED);
 ...
 * 点击面板外部时的事件
 */
Class$8.prototype.onClickOutside = function () {
  var that = this;
  var options = that.config;
  that.stopClickOutsideEvent();
  var stop = lay.onClickOutside(that.elemPicker[0], function (e) {
    var elemColorBoxSpan = that.elemColorBox.find('.' + CONST$a.PICKER_TRIG_SPAN);
    if (that.color) {
 ...
 * 窗口大小变化时自动更新位置
 */
Class$8.prototype.autoUpdatePosition = function () {
  var that = this;
  that.config;
  ...
  $win$2.on(RESIZE_EVENT_NAME, windowResizeHandler);
  that.stopResizeEvent = function () {
    $win$2.off(RESIZE_EVENT_NAME, windowResizeHandler);
    that.stopResizeEvent = $.noop;
  };
};
...
var SUPER_MOD_NAME$2 = 'element'; // 所属的超级模块名，确保向下兼容

// 创建组件
var component$b = component$e({
  name: 'tab',
  // 组件名
  ...
  render: function () {
    var that = this;
    var options = that.config;
    events$1.tabAuto(null, options.elem);
  }
});
var CONST$9 = component$b.CONST;
var $win$1 = $(window);
var $doc = $(document);
...
var events$1 = {
  ...
  tabClick: function (obj) {
    ...
    if (!obj.force) {
      var liThis = othis.siblings('.' + CONST$9.CLASS_THIS);
      var shouldChange = layui.event.call(this, SUPER_MOD_NAME$2, 'tabBeforeChange(' + filter + ')', {
        elem: parents,
        from: {
    ...
    // 执行切换
    if (!(isJump || unselect)) {
      othis.addClass(CONST$9.CLASS_THIS).siblings().removeClass(CONST$9.CLASS_THIS);
      if (hasId) {
        var contentElem = item.filter('[lay-id="' + hasId + '"]');
        contentElem = contentElem.length ? contentElem : item.eq(index);
        contentElem.addClass(CONST$9.CLASS_SHOW).siblings().removeClass(CONST$9.CLASS_SHOW);
      } else {
        item.eq(index).addClass(CONST$9.CLASS_SHOW).siblings().removeClass(CONST$9.CLASS_SHOW);
      }
    }
    layui.event.call(this, SUPER_MOD_NAME$2, 'tab(' + filter + ')', {
      elem: parents,
      index: index,
      id: hasId
    });
  ...
  tabDelete: function (obj) {
    ...
    if (!obj.force) {
      var shouldClose = layui.event.call(li[0], SUPER_MOD_NAME$2, 'tabBeforeDelete(' + filter + ')', {
        elem: tabElem,
        index: index,
        id: hasId
      });
    ...
    if (li.hasClass(CONST$9.CLASS_THIS)) {
      if (li.next()[0] && li.next().is('li')) {
        events$1.tabClick.call(li.next()[0], {
          index: index + 1
        });
      } else if (li.prev()[0] && li.prev().is('li')) {
        events$1.tabClick.call(li.prev()[0], null, index - 1);
      }
    }
    li.remove();
    if (hasId) {
    ...
    setTimeout(function () {
      events$1.tabAuto(null, tabElem);
    }, 50);
    layui.event.call(this, SUPER_MOD_NAME$2, 'tabDelete(' + filter + ')', {
      elem: tabElem,
      index: index,
      id: hasId
    });
  ...
  tabAuto: function (spread, elem) {
    ...
    targetElem.each(function () {
      var othis = $(this);
      var title = othis.children('.' + CONST$9.HEADER);
      var STOPE = 'lay-stope="tabmore"';
      var span = $('<span class="layui-unselect ' + CONST$9.BAR + '" ' + STOPE + '><i ' + STOPE + ' class="layui-icon">&#xe61a;</i></span>');
      ...
      if (allowclose && allowclose !== 'false') {
        title.find('li').each(function () {
          var li = $(this);
          if (!li.find('.' + CONST$9.CLOSE)[0] && li.attr('lay-allowclose') !== 'false') {
            var close = $('<i class="layui-icon layui-icon-close layui-unselect ' + CONST$9.CLOSE + '"></i>');
            close.on('click', function (e) {
              events$1.tabDelete.call(this, {
                e: e
              });
            });
            li.append(close);
      ...
      if (title.prop('scrollWidth') > title.outerWidth() + 1 || title.find('li').length && title.height() > function (height) {
        ...
        if (spread === 'change' && title.data('LAY_TAB_CHANGE')) {
          title.addClass(CONST$9.MORE);
        }
        if (title.find('.' + CONST$9.BAR)[0]) return;
        title.append(span);
        othis.attr('overflow', '');

        // 展开图标事件
        span.on('click', function () {
          var isSpread = title.hasClass(CONST$9.MORE);
          title[isSpread ? 'removeClass' : 'addClass'](CONST$9.MORE);
        });
      } else {
        title.find('.' + CONST$9.BAR).remove();
  ...
  hideTabMore: function (e) {
    var tsbTitle = $('.' + CONST$9.HEADER);
    if (e === true || $(e.target).attr('lay-stope') !== 'tabmore') {
      tsbTitle.removeClass(CONST$9.MORE);
      tsbTitle.find('.' + CONST$9.BAR).attr('title', '');
    }
  }
};
...
$.extend(component$b, {
  ...
  tabAdd: function (filter, options) {
    ...
    var titElem = tabElem.children('.' + CONST$9.HEADER);
    var barElem = titElem.children('.' + CONST$9.BAR);
    ...
    events$1.tabAuto(options.change ? 'change' : null, tabElem);
    return this;
  },
  /**
   * 外部 Tab 删除
  ...
  tabDelete: function (filter, layid, force) {
    ...
    events$1.tabDelete.call(liElem[0], {
      liElem: liElem,
      force: force
    });
    return this;
  ...
  tabChange: function (filter, layid, force) {
    ...
    var titElem = tabElem.children('.' + CONST$9.HEADER);
    var liElem = titElem.find('>li[lay-id="' + layid + '"]');
    events$1.tabClick.call(liElem[0], {
      liElem: liElem,
      force: force
    });
    return this;
...
$doc.on('click', '.' + CONST$9.HEADER + ' li', events$1.tabClick); // tab 头部项点击
$win$1.on('resize.lay_tab_auto_resize', events$1.tabAuto); // 自适应尺寸
...
var SUPER_MOD_NAME$1 = 'element'; // 所属的超级模块名，确保向下兼容

// 创建组件
var component$a = component$e({
  name: 'nav',
  // 组件名
  ...
  render: function () {
    ...
    var follow = function (bar, nav, index) {
      var othis = $(this);
      var child = othis.find('.' + CONST$8.NAV_CHILD);

      // 是否垂直导航菜单
      if (nav.hasClass(CONST$8.NAV_TREE)) {
        // 无子菜单时跟随
        if (!child[0]) {
        ...
        child.addClass(CONST$8.NAV_ANIM);

        // 若居中对齐
        if (child.hasClass(CONST$8.NAV_CHILD_C)) {
          child.css({
        ...
        }, device$2.ie && device$2.ie < 10 ? 0 : TIME);
        ...
        timerMore[index] = setTimeout(function () {
          child.addClass(CONST$8.CLASS_SHOW);
          othis.find('.' + CONST$8.NAV_MORE).addClass(CONST$8.NAV_MORE + 'd');
        }, 300);
      }
    };
    ...
    options.elem.each(function (index) {
      var othis = $(this);
      var bar = $('<span class="' + CONST$8.NAV_BAR + '"></span>');
      var itemElem = othis.find('.' + CONST$8.NAV_ITEM);

      // hover 滑动效果
      var hasBarElem = othis.find('.' + CONST$8.NAV_BAR);
      if (hasBarElem[0]) hasBarElem.remove();
      othis.append(bar);
      (othis.hasClass(CONST$8.NAV_TREE) ? itemElem.find('dd,>.' + CONST$8.NAV_TITLE) : itemElem).off('mouseenter.lay_nav').on('mouseenter.lay_nav', function () {
        follow.call(this, bar, othis, index);
      }).off('mouseleave.lay_nav').on('mouseleave.lay_nav', function () {
        // 鼠标移出
        // 是否为垂直导航
        if (othis.hasClass(CONST$8.NAV_TREE)) {
          bar.css({
            height: 0,
            opacity: 0
          });
          ...
          clearTimeout(timerMore[index]);
          timerMore[index] = setTimeout(function () {
            othis.find('.' + CONST$8.NAV_CHILD).removeClass(CONST$8.CLASS_SHOW);
            othis.find('.' + CONST$8.NAV_MORE).removeClass(CONST$8.NAV_MORE + 'd');
          }, 300);
        }
      });
      ...
      itemElem.find('a').each(function () {
        var thisA = $(this);
        var child = thisA.siblings('.' + CONST$8.NAV_CHILD);
        var clickEventName = 'click.lay_nav_click';

        // 输出小箭头
        if (child[0] && !thisA.children('.' + CONST$8.NAV_MORE)[0]) {
          thisA.append('<i class="layui-icon ' + CONST$8.NAV_DOWN + ' ' + CONST$8.NAV_MORE + '"></i>');
        }
...
var events = {
  ...
  clickThis: function () {
    var othis = $(this);
    var parents = othis.closest(CONST$8.NAV_ELEM);
    var filter = parents.attr('lay-filter');
    var parent = othis.parent();
    var child = othis.siblings('.' + CONST$8.NAV_CHILD);
    var unselect = typeof parent.attr('lay-unselect') === 'string'; // 是否禁用选中

    // 满足点击选中的条件
    if (!(othis.attr('href') !== 'javascript:;' && othis.attr('target') === '_blank') && !unselect) {
      if (!child[0]) {
        parents.find('.' + CONST$8.CLASS_THIS).removeClass(CONST$8.CLASS_THIS);
        parent.addClass(CONST$8.CLASS_THIS);
      }
    }

    // 若为垂直菜单
    if (parents.hasClass(CONST$8.NAV_TREE)) {
      var NAV_ITEMED = CONST$8.NAV_ITEM + 'ed'; // 用于标注展开状态
      ...
      var complete = function () {
        ...
        // 避免导航滑块错位
        parents.children('.' + CONST$8.NAV_BAR).css({
          opacity: 0
        });
      };
      ...
      // 剔除可能存在的 CSS3 动画类
      child.removeClass(CONST$8.NAV_ANIM);
      ...
      if (child[0]) {
        ...
        if (typeof parents.attr('lay-accordion') === 'string' || parents.attr('lay-shrink') === 'all') {
          ...
          parentSibs.children('.' + CONST$8.NAV_CHILD).show().stop().slideUp(ANIM_MS, complete);
        }
      }
    }
    layui.event.call(this, SUPER_MOD_NAME$1, 'nav(' + filter + ')', othis);
  }
};
var CONST$8 = component$a.CONST;
  ...
  render: function () {
    ...
    options.elem.each(function () {
      ...
      if (othis.attr('lay-showpercent')) {
        setTimeout(function () {
          elemBar.html('<span class="' + CONST$7.ELEM + '-text">' + percent + '</span>');
        }, 350);
      }
    });
  }
});
var CONST$7 = component$8.CONST;
  ...
  render: function () {
    ...
    options.elem.each(function () {
      ...
      elemItem.each(function () {
        ...
        othis[isNone ? 'removeClass' : 'addClass'](CONST$6.CLASS_SHOW);

        // 兼容旧版（ < 2.11.3）
        if (elemCont.hasClass(CONST$6.CLASS_SHOW)) {
          elemCont.removeClass(CONST$6.CLASS_SHOW);
        }

        // 点击标题
        elemTitle.off(clickEventName, event$1.titleClick).on(clickEventName, event$1.titleClick);
      });
    });
  }
});
...
var event$1 = {
  ...
  titleClick: function () {
    ...
    if (isNone) {
      ...
      thisItemElem.addClass(CONST$6.CLASS_SHOW);
    } else {
      // 先取消展开状态样式，再将元素临时显示，避免 `none` 状态导致 slideUp 动画无效
      thisItemElem.removeClass(CONST$6.CLASS_SHOW);
      thisContentElem.show().slideUp(ANIM_MS, complete);
    }

    // 是否开启手风琴
    if (isAccordion) {
      var itemSiblings = thisItemElem.siblings('.' + CONST$6.CLASS_SHOW);
      itemSiblings.removeClass(CONST$6.CLASS_SHOW);
...
const elements = {
  tab: component$b,
  nav: component$a,
  breadcrumb: component$9,
  progress: component$8,
  collapse: component$7
};

// 创建组件
const component$6 = component$e({
  name: 'element',
  // 模块名
...
$.extend(component$6, {
  ...
  tabAdd: component$b.tabAdd,
  tabDelete: component$b.tabDelete,
  tabChange: component$b.tabChange,
  tab: component$b.tab,
  progress: component$8.setValue
});
component$6.init = component$6.render;
...
// 模块名
var MOD_NAME$2 = 'upload';
var MOD_INDEX = 'layui_' + MOD_NAME$2 + '_index'; // 模块索引名
...
var upload = {
  ...
  // 事件
  on: function (events, callback) {
    return layui.onevent.call(this, MOD_NAME$2, events, callback);
  }
};
...
// 初始渲染
Class$7.prototype.render = function (rerender) {
  var that = this;
  var options = that.config;
...
//追加文件域
Class$7.prototype.file = function () {
  var that = this;
  var options = that.config;
  var elemFile = that.elemFile = $(['<input class="' + ELEM_FILE + '" type="file" accept="' + options.acceptMime + '" name="' + options.field + '"', options.multiple ? ' multiple' : '', '>'].join(''));
...
//ie8-9初始化
Class$7.prototype.initIE = function () {
  var that = this;
  var options = that.config;
  var iframe = $('<iframe id="' + ELEM_IFRAME + '" class="' + ELEM_IFRAME + '" name="' + ELEM_IFRAME + '" frameborder="0"></iframe>');
  var elemForm = $(['<form target="' + ELEM_IFRAME + '" class="' + ELEM_FORM + '" method="post" key="set-mine" enctype="multipart/form-data" action="' + options.url + '">', '</form>'].join(''));
...
//异常提示
Class$7.prototype.msg = function (content) {
...
//判断绑定元素是否为文件域本身
Class$7.prototype.isFile = function () {
...
//预读图片信息
Class$7.prototype.preview = function (callback) {
  var that = this;
...
// 执行上传
Class$7.prototype.upload = function (files, type) {
  var that = this;
  var options = that.config;
  ...
  var ajaxSend = function () {
    ...
    var request = function (sets) {
      ...
      // 追加额外的参数
      layui.each(options.data, function (key, value) {
        value = typeof value === 'function' ? sets.unified ? value() : value(sets.index, sets.file) : value;
      ...
      var opts = {
        ...
        error: function (e) {
          ...
          that.msg(text['error'] || ['Upload failed, please try again.', 'status: ' + (e.status || '') + ' - ' + (e.statusText || 'error')].join('<br>'));
  ...
  var iframeSend = function () {
    ...
    // 获取响应信息
    clearInterval(Class$7.timer);
    Class$7.timer = setInterval(function () {
      var res,
  ...
  var send = function () {
    var ready = function () {
      // IE 兼容处理
      if (device$1.ie) {
        return device$1.ie > 9 ? ajaxSend() : iframeSend();
      }
      ajaxSend();
    };
    // 上传前的回调 - 如果回调函数明确返回 false 或 Promise.reject，则停止上传
    if (typeof options.before === 'function') {
      upload.util.promiseLikeResolve(options.before(args)).then(function (result) {
        if (result !== false) {
          ready();
        } else {
          if (options.auto) {
        ...
        error !== undefined && hint$3.error(error);
      });
    } else {
      ready();
    }
  ...
  // 校验失败提示
  if (check) {
    that.msg(text['check-error'] || i18n.$t('upload.validateMessages.fileExtensionError', {
      fileType: typeName
    }));
  ...
  if (options.number && that.fileLength > options.number) {
    return that.msg(typeof text['limit-number'] === 'function' ? text['limit-number'](options, that.fileLength) : i18n.$t('upload.validateMessages.filesOverLengthLimit', {
  ...
  // 检验文件大小
  if (options.size > 0 && !(device$1.ie && device$1.ie < 10)) {
    var limitSize;
    ...
    if (limitSize) return that.msg(typeof text['limit-size'] === 'function' ? text['limit-size'](options, limitSize) : i18n.$t('upload.validateMessages.fileOverSizeLimit', {
      size: limitSize
    }));
  }
  send();
...
thisModule.getThis = function (id) {
  ...
  if (!that) hint$3.error(id ? MOD_NAME$2 + " instance with ID '" + id + "' not found" : 'ID argument required');
  return that;
};
...
var needCheckboxFallback = lay.ie && parseFloat(lay.ie) === 8 || typeof Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'checked') === 'undefined';
var Form = function () {
  this.config = {
    // 内置的验证规则
    verify: {
...
Form.prototype.getValue = function (filter, itemForm) {
  ...
  layui.each(fieldElem, function (_, item) {
    ...
    field[init_name || item.name] = this.tagName === 'SELECT' && typeof this.getAttribute('multiple') === 'string' ? othis.val() || [] : this.value;
  });
  return field;
};
...
Form.prototype.render = function (type, filter) {
  ...
  var items = {
    ...
    input: function (elem) {
      ...
      elemForm.find('input[lay-affix],textarea[lay-affix]').each(function () {
        ...
        var showAffix = function (elem, value) {
          elem = $(elem);
          if (!elem[0]) return;
          elem[$.trim(value) ? 'removeClass' : 'addClass'](HIDE$2);
        };
        ...
        var renderAffix = function (opts) {
          ...
          var elemIcon = $(function () {
            var arr = [];
            layui.each(value, function (i, item) {
              arr.push('<i class="layui-icon layui-icon-' + item + (opts.disabled ? ' ' + DISABLED : '') + '"></i>');
            });
            return arr.join('');
          }());
          ...
          if (hasElemSuffix[0]) {
            ...
            othis.css('padding-right', function () {
              var paddingRight = othis.closest('.layui-input-group')[0] ? 0 : hasElemSuffix.outerWidth();
          ...
          elemIcon.on('click', function () {
            ...
            // 对外事件
            layui.event.call(this, MOD_NAME$1, 'input-affix(' + inputFilter + ')', {
              elem: othis[0],
              affix: affix,
              options: opts
            });
    ...
    select: function (elem) {
      ...
      var events = function (reElem, titleElem, disabled, isSearch, isCreatable, isAppendTo) {
         ...
         */
        /** @type {searchOption} */
        var laySearch = select.attr('lay-search') === 'cs' ? {
          caseSensitive: true
        } : lay.options(select, {
          attr: 'lay-search'
        });
        ...
        var showDown = function () {
          if (isAppendTo) {
            ...
            reElem.appendTo(appendTarget).css({
            ...
            if (resizeObserver$1) {
              resizeObserver$1.observe(reElem[0], updatePosition);
            }
          }
          ...
          dds.removeClass(HIDE$2);
          dts.removeClass(HIDE$2);
          ...
          removeClickOutsideEvent = lay.onClickOutside(isAppendTo ? reElem[0] : dl[0], function () {
            hideDown();
        ...
        var hideDown = function (choose) {
          ...
          if (isAppendTo) {
            ...
            if (resizeObserver$1) {
              resizeObserver$1.unobserve(reElem[0]);
            }
          }
          if (choose) return;
        ...
        input.on('keyup', function (e) {
          ...
          var setThisDd = function (prevNext) {
            e.preventDefault();
            var allDisplayedElem = dl.children('dd:not(.' + HIDE$2 + ',.' + DISABLED + ')');
            ...
            var nextIndex = prevNext === 'prev' ? selectedIndex - 1 < firstIndex ? lastIndex : selectedIndex - 1 : selectedIndex + 1 > lastIndex ? firstIndex : selectedIndex + 1;
        ...
        var notOption = function (value, callback, origin) {
          ...
          layui.each(dds, function () {
            ...
            origin === 'keyup' && othis[(isCreatable ? not && !isCreateOption : not) ? 'addClass' : 'removeClass'](HIDE$2);
          });
          // 处理 select 分组元素
          origin === 'keyup' && layui.each(dts, function () {
            ...
            var allHide = thisDds.length == thisDds.filter('.' + HIDE$2).length; // 当前分组下所有dd元素都隐藏了
            othis[allHide ? 'addClass' : 'removeClass'](HIDE$2);
          });
        ...
        dl.on('click', 'dd', function () {
          ...
          layui.event.call(this, MOD_NAME$1, 'select(' + filter + ')', {
            elem: select[0],
            value: value,
            othis: reElem
          });
      ...
      selects.each(function (index, select) {
        ...
        var placeholder = optionsFirst ? optionsFirst.value ? TIPS : optionsFirst.text || TIPS : TIPS;
    ...
    checkbox: function (elem) {
      ...
      var events = function (reElem, RE_CLASS) {
        ...
        check.off(clickEventName).on(clickEventName, function () {
          ...
          // 事件
          layui.event.call(check[0], MOD_NAME$1, RE_CLASS[2] + '(' + filter + ')', {
            elem: check[0],
            value: check[0].value,
            othis: reElem
          });
        ...
        that.syncAppearanceOnPropChanged(this, 'checked', function () {
          if (isSwitch) {
            var title = (reElem.next('*[lay-checkbox]')[0] ? reElem.next().html() : check.attr('title') || '').split('|');
      ...
      checks.each(function (index, check) {
        ...
        var title = util.escape($.trim(check.title || function () {
          // 向下兼容 lay-text 属性
        ...
        // 替代元素
        var reElem = $(['<div class="layui-unselect ' + RE_CLASS[0], check.checked ? ' ' + RE_CLASS[1] : '',
        // 选中状态
        disabled ? ' layui-checkbox-disabled ' + DISABLED : '',
        // 禁用状态
        '"', ' lay-skin="' + skin + '"' ,
        // 风格
        '>', function () {
          ...
          var type = {
            // 复选框
            checkbox: [title[0] ? '<div ' + titleTplAttrs + '>' + title[0] + '</div>' : skin === 'primary' ? '' : '<div></div>', '<i class="layui-icon ' + (skin === 'primary' && !check.checked && othis.get(0).indeterminate ? CLASS.SUBTRA : 'layui-icon-ok') + '"></i>'].join(''),
            // 开关
    ...
    radio: function (elem) {
      ...
      var events = function (reElem) {
        ...
        radio.off(clickEventName).on(clickEventName, function () {
          ...
          layui.event.call(radio[0], MOD_NAME$1, 'radio(' + filter + ')', {
            elem: radio[0],
            value: radio[0].value,
            othis: reElem
          });
        ...
        that.syncAppearanceOnPropChanged(this, 'checked', function () {
          ...
          if (radioEl.checked) {
            ...
            var forms = radio.parents(ELEM$1);
      ...
      radios.each(function (index, radio) {
        ...
        // 禁用状态
        skin ? ' lay-skin="' + skin + '"' : '', '>', '<i class="layui-anim layui-icon ' + ICON[radio.checked ? 0 : 1] + '"></i>', '<div ' + titleTplAttrs + '>' + title + '</div>', '</div>'].join(''));
        othis.after(reElem);
  ...
  if (layui.type(type) === 'object') {
    ...
    type ? items[type] ? items[type]() : hint$2.error('[form] "' + type + '" is an unsupported form element type') : renderItem();
  }
  return that;
};
...
Form.prototype.syncAppearanceOnPropChanged = function () {
  ...
  if (needCheckboxFallback) {
    return function (elem, propName, handler) {
      ...
      Object.defineProperty(elem, propName, lay.extend({}, originProps, {
        // 此处的 get 是为了兼容 IE<9
        get: function () {
...
Form.prototype.validate = function (elem) {
  ...
  layui.each(elem, function (_, item) {
    ...
    layui.each(vers, function (_, thisVer) {
      ...
      // 匹配验证规则
      if (rule) {
        verst = typeof rule === 'function' ? errorText = rule(value, item) : !rule[0].test(value); // 兼容早期数组中的正则写法
...
var submit = Form.prototype.submit = function (filter, callback) {
  ...
  // 表单域 lay-filter 属性值
  var layFilter = typeof filter === 'string' ? filter : button.attr('lay-filter');

  // 当前所在表单域
  var elem = this.getFormElem ? this.getFormElem(layFilter) : button.parents(ELEM$1).eq(0);
  ...
  var params = {
    ...
    // 触发事件的对象
    form: this.getFormElem ? elem[0] : button.parents('form')[0],
    // 当前所在的 form 元素，如果存在的话
    field: field // 当前表单数据
  };
  ...
  // 事件
  return layui.event.call(this, MOD_NAME$1, 'submit(' + layFilter + ')', params);
};
...
// 表单提交事件
$dom.on('submit', ELEM$1, submit).on('click', '*[lay-submit]', submit);
...
var getThisTable$1 = function (id) {
  ...
  if (!that) hint$1.error(id ? "The table instance with ID '" + id + "' not found" : 'ID argument required');
  return that || null;
};
...
var getThisTableConfig = function (id) {
  ...
  if (!config) hint$1.error(id ? "The table instance with ID '" + id + "' not found" : 'ID argument required');
  return config || null;
};
...
var parseTempData = function (obj) {
  ...
  // 获取模板内容
  if (templet) {
    content = typeof templet === 'function' ? templet.call(item3, obj.tplData, obj.obj) : laytpl(resolveTplStr(templet) || String(content)).render($.extend({
      LAY_COL: item3
    }, obj.tplData));
  }
...
var TPL_HEADER = function (options) {
  ...
  return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{=d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{=d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}', function () {
    ...
    return '';
  }(), '{{# var isSort = !(item2.colGroup) && item2.sort; }}', '<th data-field="{{= item2.field||i2 }}" data-key="{{=d.index}}-{{=i1}}-{{=i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{= item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{=item2.minWidth}}"{{# } }} {{# if(item2.maxWidth){ }}data-maxwidth="{{=item2.maxWidth}}"{{# } }} {{# if(item2.style){ }}style="{{=item2.style}}"{{# } }} ' + rowCols + ' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}"{{# if(item2.title){ }} title="{{ layui.$(\'<div>\' + item2.title + \'</div>\').text() }}"{{# } }}>', '<div class="layui-table-cell laytable-cell-', '{{# if(item2.colGroup){ }}', 'group', '{{# } else { }}', '{{=d.index}}-{{=i1}}-{{=i2}}', '{{# if(item2.type !== "normal"){ }}', ' laytable-cell-{{= item2.type }}', '{{# } }}', '{{# } }}', '" {{#if(item2.align){}}align="{{=item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}',
  //复选框
  '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', '{{# } else { }}', '<span>{{-item2.title||""}}</span>', '{{# if(isSort){ }}', '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="{{= d.i18nMessages.table_sort_asc }}"></i><i class="layui-edge layui-table-sort-desc" title="{{= d.i18nMessages.table_sort_desc }}"></i></span>', '{{# } }}', '{{# } }}', '</div>', '</th>', options.fixed ? '{{# }; }}' : '', '{{# }); }}', '</tr>', '{{# }); }}', '</thead>', '</table>'].join('');
};

// tbody 区域模板
var TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{=d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{=d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<tbody></tbody>', '</table>'].join('');

// 主模板
var TPL_MAIN = ['{{# if(d.data.toolbar){ }}', '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', '</div>', '{{# } }}', '<div class="layui-table-box">', '{{# if(d.data.loading){ }}', '<div class="layui-table-init">', '<div class="layui-table-loading-icon">', '{{# if(typeof d.data.loading === "string"){ }}', '{{- d.data.loading}}', '{{# } else{ }}', '<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>', '{{# } }}', '</div>', '</div>', '{{# } }}', '{{# var left, right; }}', '<div class="layui-table-header">', TPL_HEADER(), '</div>', '<div class="layui-table-body layui-table-main">', TPL_BODY, '</div>', '{{# if(left){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', TPL_HEADER({
  fixed: true
}), '</div>', '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '{{# if(right){ }}', '<div class="layui-table-fixed layui-table-fixed-r layui-hide">', '<div class="layui-table-header">', TPL_HEADER({
  fixed: 'right'
}), '<div class="layui-table-mend"></div>', '</div>', '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '</div>', '{{# if(d.data.totalRow){ }}', '<div class="layui-table-total">', '<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{=d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{=d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<tbody><tr><td><div class="layui-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>', '</table>', '</div>', '{{# } }}', '<div class="layui-table-column layui-table-page layui-hide">', '<div class="layui-inline layui-table-pageview" id="layui-table-page{{=d.index}}"></div>', '</div>'].join('');
var _WIN = $(window);
var _DOC = $(document);
...
Class$6.prototype.config = {
  ...
  // 显示默认上下文菜单
  autoSort: true,
  // 是否前端自动排序。如果否，则需自主排序（通常为服务端处理好排序）
  cols: []
};
...
Class$6.prototype.render = function (type) {
  ...
  // 开始插入替代元素
  var othis = options.elem;
  var hasRender = othis.next('.' + ELEM_VIEW$1);
  ...
  reElem.addClass(function () {
    var arr = [ELEM_VIEW$1, ELEM_VIEW$1 + '-' + that.index, 'layui-form', 'layui-border-box'];
  ...
  that.layMain = reElem.find(ELEM_MAIN$1);
...
Class$6.prototype.setInit = function (type) {
  ...
  // 初始化 css 参数
  if (options.css && options.css.indexOf(ELEM_VIEW$1) === -1) {
    var css = options.css.split('}');
    layui.each(css, function (index, value) {
      if (value) {
        css[index] = '.' + ELEM_VIEW$1 + '-' + that.index + ' ' + value;
      }
    });
...
Class$6.prototype.renderStyle = function () {
  ...
  (function (lineStyle) {
    ...
    // 多行相关样式
    layui.each(['{' + lineStyle + '}', '.layui-table-cell{height: auto; max-height: ' + cellMaxHeight + '; white-space: normal; text-overflow: clip;}', '> td:hover > .layui-table-cell{overflow: auto;}'].concat(device.ie ? ['.layui-table-edit{height: ' + cellMaxHeight + ';}', 'td[data-edit]:hover:after{height: ' + cellMaxHeight + ';}'] : []), function (i, val) {
  ...
  var defaultConfig = {
        ...
        openPanel({
          list: function () {
            ...
            form.on('checkbox(LAY_TABLE_TOOL_COLS)', function (obj) {
              ...
              // 同步勾选列的 hide 值和隐藏样式
              col.hide = !checked;
              that.elem.find('*[data-key="' + key + '"]')[checked ? 'removeClass' : 'addClass'](HIDE$1);
    ...
    exports: {
      ...
      onClick: function (obj) {
        ...
        var openPanel = obj.openPanel;
        var elem = obj.elem;
        if (!data.length) return layer.tips(i18n.$t('table.tools.export.noDataPrompt'), elem, {
          tips: 3
        });
        if (device.ie) {
          ...
          openPanel({
            list: function () {
              return ['<li data-type="csv">' + i18n.$t('table.tools.export.csvText') + '</li>'].join('');
            }(),
    ...
    print: {
      ...
      onClick: function (obj) {
        ...
        obj.config;
        var elem = obj.elem;
        if (!data.length) return layer.tips(i18n.$t('table.tools.print.noDataPrompt'), elem, {
          tips: 3
        });
        var printWin = window.open('about:blank', '_blank');
        var style = ['<style>', 'body{font-size: 12px; color: #5F5F5F;}', 'table{width: 100%; border-collapse: collapse; border-spacing: 0;}', 'th,td{line-height: 20px; padding: 9px 15px; border: 1px solid #ccc; text-align: left; font-size: 12px; color: #5F5F5F;}', 'a{color: #5F5F5F; text-decoration:none;}', 'img{max-height: 100%;}', '*.layui-hide{display: none}', '</style>'].join('');
...
Class$6.prototype.setGroupWidth = function (th) {
  ...
  (th ? 'th[data-key=' + th.data('parentkey') + ']>' : '') + '.' + ELEM_GROUP); // 若无指向当前活动表头，则自下而上获取所有组合表头
...
Class$6.prototype.setColsWidth = function (opt) {
  ...
  var getAutoWidth = function (back) {
    ...
    // 如果未填充满，则将剩余宽度平分
    cntrWidth > countWidth && autoColNums > 0 && (autoWidth = (cntrWidth - countWidth) / autoColNums);
  };
  getAutoWidth();
    ...
    var ajaxOptions = {
      ...
      success: function (res) {
        ...
        if (res[response.statusName] != response.statusCode) {
          that.errorView(res[response.msgName] || i18n.$t('table.dataFormatError', {
      ...
      error: function (e, msg) {
        ...
        that.errorView(i18n.$t('table.xhrError', {
          msg: msg
        }));
    ...
    that.eachCols(function (i3, item3) {
      ...
      var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () {
        ...
        switch (item3.type) {
          ...
          case 'radio':
            // 单选
            return '<input type="radio" name="layTableRadio_' + options.index + '" ' + function () {
              var arr = [];
              if (tplData[checkName]) arr[0] = 'checked';
              if (tplData[disabledName]) arr.push('disabled');
              return arr.join(' ');
...
Class$6.prototype.renderTotal = function (data, totalRowData) {
  ...
  that.eachCols(function (i3, item3) {
    ...
    var thisTotalNum = totalNums[field] ? parseFloat(totalNums[field] || 0).toFixed(decimals) : '';
...
Class$6.prototype.setRowChecked = function (opts) {
  ...
  (opts.type === 'radio' && fixRElem.hasClass(HIDE$1) ? checkedElem.first() : checkedElem).prop('checked', getChecked(checkedSameElem.prop('checked')));
  that.syncCheckAll();
  if (isCheckAllOrMult) {
    setTimeout(function () {
...
// 数据排序
Class$6.prototype.sort = function (opts) {
  ...
  try {
    ...
    hint$1.error("Table modules: sort field '" + field + "' not matched");
  }
...
Class$6.prototype.scrollPatch = function () {
  ...
  that.layFixed.find(ELEM_BODY$1).css('height', layMainTable.height() >= fixHeight ? fixHeight : 'auto').scrollTop(that.layMain.scrollTop()); // 固定列滚动条高度

  // 表格宽度小于容器宽度时，隐藏固定列
  that.layFixRight[table.cache[that.key] && table.cache[that.key].length && outWidth > 0 ? 'removeClass' : 'addClass'](HIDE$1);
...
Class$6.prototype.events = function () {
    ...
    _DOC.on('mousemove', function (e) {
      if (thisTable.eventMoveElem) {
        ...
        if (dict.rule) {
          ...
          var id = thisTable.eventMoveElem.closest('.' + ELEM_VIEW$1).attr(MOD_ID$1);
      ...
      if (thisTable.eventMoveElem) {
        ...
        var id = th.closest('.' + ELEM_VIEW$1).attr(MOD_ID$1);
        var thatTable = getThisTable$1(id);
        if (!thatTable) return;
        var key = th.data('key');
  ...
  that.elem.on('click', 'input[lay-type="layTableRadio"]+', function (e) {
    ...
    // 事件
    layui.event.call(radio[0], MOD_NAME, 'radio(' + filter + ')', commonMember.call(radio[0], {
      checked: checked,
      getCol: function () {
        // 获取当前列的表头配置信息
        return that.col(td.data('key'));
  ...
  var setRowEvent = function (eventType, e) {
    ...
    if (eventType !== 'rowContextmenu') {
      var UNROW = ['.layui-form-checkbox', '.layui-form-switch', '.layui-form-radio', '[lay-unrow]', '[lay-type="layTableCheckbox"]', '[lay-type="layTableRadio"]'].join(',');
  ...
  var showGridExpandIcon = function (hide) {
    ...
    } else if ((elemCell.prop('scrollWidth') > elemCell.prop('clientWidth') || elemCell.find('br').length > 0) && !options.lineStyle) {
  ...
  var gridExpand = function (e, expandedMode) {
    ...
    if (expandedMode === 'tips') {
      // TIPS 展开风格
      that.tipsIndex = layer.tips(['<div class="layui-table-tips-main" style="margin-top: -' + (elemCell.height() + 23) + 'px;' + function () {
        if (options.size === 'sm') {
  ...
  var toolFn = function (type) {
    ...
    // 执行事件
    layui.event.call(this, MOD_NAME, (type || 'tool') + '(' + filter + ')', commonMember.call(this, {
...
Class$6.prototype.getContentWidth = function (elem) {
  ...
  lay.ie && elem.css('box-sizing') === 'border-box' || elem.css('display') === 'none') {
    return elem.width();
  }
  ...
  if (typeof size === 'undefined' || !size.width) {
    ...
    return size.boxSizing === 'border-box' ? size.width - size.paddingLeft - size.paddingRight - size.borderLeftWidth - size.borderRightWidth : size.width;
  }
};
Class$6.prototype.dispose = function () {
  var that = this;
...
Class$6.prototype.calcFixedRowHeight = function () {
  var that = this;
...
Class$6.prototype.fixedRowHeightPatchOnHover = function (targetEl, trsElem, isEnter) {
  var that = this;
  var style = that.elem.children('style')[0];
...
Class$6.prototype.observeResize = function () {
  var that = this;
...
table.init = function (filter, settings) {
  settings = settings || {};
  var that = this;
  var elemTable = typeof filter === 'object' ? filter : typeof filter === 'string' ? $('table[lay-filter="' + filter + '"]') : $(ELEM + '[lay-data], ' + ELEM + '[lay-options]');
...
table.exportFile = function (id, data, opts) {
  ...
  if (device.ie) return hint$1.error('IE_NOT_SUPPORT_EXPORTS');
      ...
      if (typeof id === 'object') {
        ...
        table.eachCols(id, function (i3, item3) {
          if (item3.ignoreExport === false || item3.field && item3.type == 'normal') {
            ...
            if (item3.hide && item3.ignoreExport !== false || item3.ignoreExport === true // 忽略导出
            ) {
  ...
  if (layui.type(cols) === 'boolean') {
    ...
    that.eachCols(function (i2, item2) {
      ...
      if (col.hide != cols) {
        ...
        that.elem.find('*[data-key="' + key + '"]')[hide ? 'addClass' : 'removeClass'](HIDE$1);
        // 根据列的显示隐藏，同步多级表头的父级相关属性值
...
table.reloadData = function () {
  ...
  var dataParams = new RegExp('^(' + ['elem', 'id', 'cols', 'width', 'height', 'maxHeight', 'toolbar', 'defaultToolbar', 'className', 'css', 'pagebar'].join('|') + ')$');
...
Class$5.prototype.config = {
  tree: {
    ...
    view: {
      ...
      // 是否显示图标(节点类型图标)
      icon: '',
      // 节点图标，如果设置了这个属性或者数据中有这个字段信息，不管打开还是关闭都以这个图标的值为准
    ...
    async: {
      enable: false,
      // 是否开启异步加载模式，只有开启的时候其他参数才起作用
      url: '',
      // 异步加载的接口，可以根据需要设置与顶层接口不同的接口，如果相同可以不设置该参数
      type: null,
      // 请求的接口类型，设置可缺省同上
      contentType: null,
      // 提交参数的数据类型，设置可缺省同上
    ...
    callback: {
      beforeExpand: null,
      // 展开前的回调 return false 可以阻止展开的动作
      onExpand: null // 展开之后的回调
    }
  }
};
Class$5.prototype.normalizedIcon = function (iconStr) {
  return iconStr ? HTML_TAG_RE.test(iconStr) ? iconStr : '<i class="' + iconStr + '"></i>' : '';
};
  ...
  if (trExpand) {
    ...
    if (trExpanded) {
      ...
      if (asyncSetting.enable && trData[isParentKey] && (!trData[LAY_ASYNC_STATUS] || trData[LAY_ASYNC_STATUS] === 'error')) {
        ...
        if (asyncContentType && asyncContentType.indexOf('application/json') == 0) {
          // 提交 json 格式
          data = JSON.stringify(data);
        }
...
Class$5.prototype.updateNodeIcon = function (opts) {
  ...
  flexIconElem.css('visibility', isParent || treeOptions.view.showFlexIconIfNotParent ? 'visible' : 'hidden').html(isExpand ? treeOptions.view.flexIconOpen : treeOptions.view.flexIconClose);
  // 处理节点图标
  if (treeOptions.view.showIcon) {
    var nodeIconElem = scopeEl.find('.layui-table-tree-nodeIcon:not(.layui-table-tree-iconCustom)');
    var nodeIcon = isParent ? isExpand ? treeOptions.view.iconOpen : treeOptions.view.iconClose : treeOptions.view.iconLeaf;
    nodeIconElem.toggleClass('layui-table-tree-iconLeaf', !isParent).html(nodeIcon);
  }
};
Class$5.prototype.renderTreeTable = function (tableView, level, sonSign) {
  ...
  layui.each(tableView.find('td[data-field="' + nameKey + '"]'), function (index, item) {
    ...
    var flexIconElem = itemCell.html(['<div class="layui-inline layui-table-tree-flexIcon" ', 'style="', 'margin-left: ' + indent * trElem.attr('data-level') + 'px;', trData[isParentKey] || treeOptionsView.showFlexIconIfNotParent ? '' : ' visibility: hidden;', '">', trData[LAY_EXPAND] ? treeOptionsView.flexIconOpen : treeOptionsView.flexIconClose,
    // 折叠图标
    '</div>', treeOptionsView.showIcon ? '<div class="layui-inline layui-table-tree-nodeIcon' + (trData[customName.icon] || treeOptionsView.icon ? ' layui-table-tree-iconCustom' : '') + (trData[isParentKey] ? '' : ' layui-table-tree-iconLeaf') + '">' + (that.normalizedIcon(trData[customName.icon]) || treeOptionsView.icon || (trData[isParentKey] ? trData[LAY_EXPAND] ? treeOptionsView.iconOpen : treeOptionsView.iconClose : treeOptionsView.iconLeaf) || '') + '</div>' : '',
    // 区分父子节点
...
treeTable.removeNode = function (id, node, _keepParent) {
  ...
  layui.each(tableCache, function (i4, item4) {
    tableView.find('tr[data-level="0"][lay-data-index="' + item4[LAY_DATA_INDEX] + '"]').attr('data-index', i4).data('index', i4);
  });
  options.hasNumberCol && formatNumber(that);
  // 更新父节点状态
  if (nodeP) {
...
var component$5 = component$e({
  ...
  render: function () {
    ...
    // 标签页元素项
    that.headerElem = ['.' + component$5.CONST.HEADER + ':eq(0)', '>li'];
    that.bodyElem = ['.' + component$5.CONST.BODY + ':eq(0)', '>.' + component$5.CONST.ITEM];
    ...
    // 清除隐藏占位
    if (options.elem.hasClass(component$5.CONST.CLASS_HIDEV)) {
      options.elem.removeClass(component$5.CONST.CLASS_HIDEV);
    }
    ...
    // 渲染成功后的事件
    layui.event.call(options.elem[0], component$5.CONST.MOD_NAME, 'afterRender(' + options.id + ')', data);
  },
  // 事件
  events: function () {
    var that = this;
    ...
    var MOD_NAME = component$5.CONST.MOD_NAME;
    ...
    if (!inner.onresize) {
      ...
      $(window).on('resize', function () {
        clearTimeout(timer);
        timer = setTimeout(function () {
          layui.each(component$5.cache.id, function (key) {
            var that = component$5.getInst(key);
            if (!that) return;
            that.roll('init');
          });
        }, 50);
...
Class$4.prototype.add = function (opts) {
  ...
  // 回调
  typeof opts.done === 'function' && opts.done($.extend(data, {
...
Class$4.prototype.close = function (thisHeaderItem, force) {
  ...
  if (!force) {
    var closable = layui.event.call(thisHeaderItem[0], component$5.CONST.MOD_NAME, 'beforeClose(' + options.id + ')', $.extend(data, {
      index: index
    }));
  ...
  if (thisHeaderItem.hasClass(component$5.CONST.CLASS_THIS)) {
  ...
  // 标签关闭后的事件
  layui.event.call(data.thisHeaderItem[0], component$5.CONST.MOD_NAME, 'afterClose(' + options.id + ')', data);
};
...
Class$4.prototype.change = function (thisHeaderItem, force) {
  ...
  // 标签关闭前的事件。若非强制关闭，可则根据事件的返回结果决定是否关闭
  if (!force) {
    var enable = layui.event.call(thisHeaderItem[0], component$5.CONST.MOD_NAME, 'beforeChange(' + options.id + ')', $.extend(data, {
      from: {
        index: data.index,
  ...
  // 执行标签头部切换
  thisHeaderItem.addClass(component$5.CONST.CLASS_THIS).siblings().removeClass(component$5.CONST.CLASS_THIS);

  // 执行标签内容切换
  that.findBodyItem(layid || index).addClass(component$5.CONST.CLASS_SHOW).siblings().removeClass(component$5.CONST.CLASS_SHOW);
  that.roll('auto', index);
  ...
  // 标签切换后的事件
  layui.event.call(data.thisHeaderItem[0], component$5.CONST.MOD_NAME, 'afterChange(' + options.id + ')', data);
};
...
Class$4.prototype.renderBodyItem = function (opts) {
  var that = this;
  var options = that.config;
  var bodyItem = $(opts.bodyItem || options.bodyItem || '<div class="' + component$5.CONST.ITEM + '"></div>');
...
Class$4.prototype.appendClose = function (headerItem, opts) {
  ...
  // 可关闭项追加关闭按钮
  if (!headerItem.find('.' + component$5.CONST.CLOSE)[0]) {
    var close = $('<i class="layui-icon layui-icon-close layui-unselect ' + component$5.CONST.CLOSE + '"></i>');
...
Class$4.prototype.renderClose = function () {
  ...
  container.header.items.each(function () {
    ...
    if (options.closable) {
      ...
      $this.find('.' + component$5.CONST.CLOSE).remove();
    }
  });
};
...
Class$4.prototype.roll = function (mode, index) {
  ...
  var rollToVisibleArea = function () {
    ...
    // 当选中标签溢出在可视区域「右侧」时，
    countWidth = thisLeft + thisItemElem.outerWidth() + (thisItemElem.next().outerWidth() || 0) + padding; // 始终空出下一个标签
  ...
  var rollElem = {
    ...
    bar: $(['<div class="' + CLASS_BAR + '">', '<i class="layui-icon ' + CLASS_BAR_ICON[0] + '" lay-mode="prev"></i>', '<i class="layui-icon ' + CLASS_BAR_ICON[1] + '" lay-mode="next"></i>', '</div>'].join(''))
  };
  ...
  if (scrollMode || !scrollMode && scrollWidth > outerWidth) {
    if (!elemScroll[0]) {
      if (options.elem.hasClass(component$5.CONST.CARD)) {
        rollElem.elem.addClass(component$5.CONST.CARD);
      }
      ...
      rollElem.bar.children().on('click', function () {
        ...
        if ($(this).hasClass(component$5.CONST.CLASS_DISABLED)) return;
        mode && that.roll(mode);
      });
    }
  } else if (!scrollMode) {
  ...
  // 左
  elemScroll.find('.' + CLASS_BAR_ICON[0])[tabsLeft < 0 ? 'removeClass' : 'addClass'](component$5.CONST.CLASS_DISABLED);
  // 右
  elemScroll.find('.' + CLASS_BAR_ICON[1])[parseFloat(tabsLeft + scrollWidth) - outerWidth > 0 ? 'removeClass' : 'addClass'](component$5.CONST.CLASS_DISABLED);
};
...
Class$4.prototype.data = function () {
  ...
  var options = that.config;
  var container = that.getContainer();
  var thisHeaderItem = container.header.items.filter('.' + component$5.CONST.CLASS_THIS);
  var index = thisHeaderItem.index();
  var layid = thisHeaderItem.attr('lay-id');
  return {
    options: options,
    ...
    container: container,
    // 标签容器的相关元素
    thisHeaderItem: thisHeaderItem,
    // 当前活动标签头部项
    thisBodyItem: that.findBodyItem(layid || index),
    // 当前活动标签内容项
    index: index,
    // 当前活动标签索引
...
$.extend(component$5, {
  ...
  add: function (id, opts) {
    var that = component$5.getInst(id);
    if (!that) return;
    that.add(opts);
  },
  /**
...
var component$4 = component$e({
  ...
  render: function () {
    ...
    if (that.elem.find('.' + CONST$5.ELEM_SET).length == 0) {
    ...
    that.elem.find('.' + CONST$5.ELEM_SET).each(function () {
      var othis = $(this);
      ...
      if (!othis.next()[0] && othis.parents('.layui-tree-pack').eq(1).hasClass('layui-tree-lineExtend')) {
        othis.addClass(CONST$5.ELEM_LINE_SHORT);
      }

      // 没有下一个节点 外层最后一个
      if (!othis.next()[0] && !othis.parents('.' + CONST$5.ELEM_SET).eq(0).next()[0]) {
        othis.addClass(CONST$5.ELEM_LINE_SHORT);
      }
    });
  },
...
var CONST$5 = component$4.CONST;

/**
 * 扩展组件原型方法
 */

var Class$3 = component$4.Class;

// 重载实例
Class$3.prototype.reload = function (options, type) {
  var that = this;
...
// 渲染表单
Class$3.prototype.renderForm = function (type) {
  form.render(type, 'LAY-tree-' + this.index);
};

// 节点解析
Class$3.prototype.tree = function (elem, children) {
  var that = this;
  var options = that.config;
  var customName = options.customName;
  ...
  layui.each(data, function (index, item) {
    ...
    var entryDiv = $(['<div data-id="' + item[customName.id] + '" class="' + [CONST$5.ELEM_SET, item.spread ? 'layui-tree-spread' : ''].join(' ') + '">', '<div class="layui-tree-entry">', '<div class="layui-tree-main">',
    // 箭头
    function () {
      if (options.showLine) {
        if (hasChild) {
        ...
        return '<span class="layui-tree-iconClick"><i class="layui-tree-iconArrow ' + (hasChild ? '' : CONST$5.CLASS_HIDE) + '"></i></span>';
      }
    }(),
    // 复选框
    function () {
    ...
    function () {
      if (options.isJump && item.href) {
        return '<a href="' + item.href + '" target="_blank" class="' + CONST$5.ELEM_TEXT + '">' + (item[customName.title] || item.label || options.text.defaultNodeName) + '</a>';
      } else {
        return '<span class="' + CONST$5.ELEM_TEXT + (item.disabled ? ' ' + CONST$5.CLASS_DISABLED : '') + '">' + (item[customName.title] || item.label || options.text.defaultNodeName) + '</span>';
      }
    }(), '</div>',
    // 节点操作图标
    function () {
    ...
    // 若有前置节点，前置节点加连接线
    if (entryDiv.prev('.' + CONST$5.ELEM_SET)[0]) {
...
// 展开节点
Class$3.prototype.spread = function (elem, item) {
  var that = this;
  var options = that.config;
  var entry = elem.children('.' + CONST$5.ELEM_ENTRY);
  var elemMain = entry.children('.' + CONST$5.ELEM_MAIN);
  var elemCheckbox = elemMain.find('input[same="layuiTreeCheck"]');
  var elemIcon = entry.find('.' + CONST$5.ICON_CLICK);
  var elemText = entry.find('.' + CONST$5.ELEM_TEXT);
  var touchOpen = options.onlyIconControl ? elemIcon : elemMain; // 判断展开通过节点还是箭头图标
  var state = '';

  // 展开收缩
  touchOpen.on('click', function () {
    var packCont = elem.children('.' + CONST$5.ELEM_PACK);
    ...
    if (!packCont[0]) {
      ...
      if (elem.hasClass(CONST$5.ELEM_SPREAD)) {
        elem.removeClass(CONST$5.ELEM_SPREAD);
        packCont.slideUp(200);
        iconClick.removeClass(CONST$5.ICON_SUB).addClass(CONST$5.ICON_ADD);
        that.updateFieldValue(item, 'spread', false);
      } else {
        elem.addClass(CONST$5.ELEM_SPREAD);
        packCont.slideDown(200);
        iconClick.addClass(CONST$5.ICON_SUB).removeClass(CONST$5.ICON_ADD);
        that.updateFieldValue(item, 'spread', true);

        // 是否手风琴
        if (options.accordion) {
          var sibls = elem.siblings('.' + CONST$5.ELEM_SET);
          sibls.removeClass(CONST$5.ELEM_SPREAD);
          sibls.children('.' + CONST$5.ELEM_PACK).slideUp(200);
          sibls.find('.layui-tree-icon').children('.layui-icon').removeClass(CONST$5.ICON_SUB).addClass(CONST$5.ICON_ADD);
        }
      }
    }
  });
  ...
  elemText.on('click', function () {
    ...
    // 判断是否禁用状态
    if (othis.hasClass(CONST$5.CLASS_DISABLED)) return;
...
Class$3.prototype.updateFieldValue = function (obj, field, value) {
  if (field in obj) {
    obj[field] = value;
  }
};
 ...
 * @returns
 */
Class$3.prototype.syncCheckedState = function (elemCheckbox, item, isManual) {
  var that = this;
  var options = that.config;
  var customName = options.customName;
  var checked = elemCheckbox.prop('checked');
  var nodeWrapper = elemCheckbox.closest('.' + CONST$5.ELEM_SET);
  ...
  var setChildrenChecked = function (thisNodeElem, item) {
    ...
    var childrenPack = thisNodeElem.children('.' + CONST$5.ELEM_PACK);
    var childrenWrapper = childrenPack.children('.' + CONST$5.ELEM_SET);
    var elemCheckboxs = childrenWrapper.children('.' + CONST$5.ELEM_ENTRY).find('input[same="layuiTreeCheck"]');
  ...
  var setParentsChecked = function (thisNodeElem) {
    // 若无父节点，则终止递归
    if (!thisNodeElem.parents('.' + CONST$5.ELEM_SET)[0]) return;
    var descendantsChecked; // 后代节点选中状态
    var parentPack = thisNodeElem.parent('.' + CONST$5.ELEM_PACK);
...
// 复选框选择
Class$3.prototype.checkClick = function (elem, item) {
  var that = this;
  var options = that.config;
  var entry = elem.children('.' + CONST$5.ELEM_ENTRY);
  var elemMain = entry.children('.' + CONST$5.ELEM_MAIN);
...
// 节点操作
Class$3.prototype.operate = function (elem, item) {
  var that = this;
  var options = that.config;
  var customName = options.customName;
  var entry = elem.children('.' + CONST$5.ELEM_ENTRY);
  ...
  entry.children('.layui-tree-btnGroup').on('click', '.layui-icon', function (e) {
    ...
    if (type == 'add') {
      ...
      if (!packCont[0]) {
        // 若开启连接线，更改图标样式
        if (options.showLine) {
          elemMain.find('.' + CONST$5.ICON_CLICK).addClass('layui-tree-icon');
          elemMain.find('.' + CONST$5.ICON_CLICK).children('.layui-icon').addClass(CONST$5.ICON_ADD).removeClass('layui-icon-leaf');
          // 若未开启连接线，显示箭头
        } else {
          elemMain.find('.layui-tree-iconArrow').removeClass(CONST$5.CLASS_HIDE);
        }
        // 节点添加子节点容器
      ...
      that.tree(elem.children('.' + CONST$5.ELEM_PACK), [obj]);
      ...
      if (options.showLine) {
        ...
        if (!packCont[0]) {
          // 遍历兄弟节点，判断兄弟节点是否有子节点
          var siblings = elem.siblings('.' + CONST$5.ELEM_SET);
          var num = 1;
          var parentPack = elem.parent('.' + CONST$5.ELEM_PACK);
          layui.each(siblings, function (index, i) {
            if (!$(i).children('.' + CONST$5.ELEM_PACK)[0]) {
              num = 0;
            }
          });
          ...
          if (num == 1) {
            // 兄弟节点添加连接线
            siblings.children('.' + CONST$5.ELEM_PACK).addClass(CONST$5.ELEM_SHOW);
            siblings.children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).removeClass(CONST$5.ELEM_LINE_SHORT);
            elem.children('.' + CONST$5.ELEM_PACK).addClass(CONST$5.ELEM_SHOW);
            // 父级移除延伸线
            parentPack.removeClass(CONST$5.ELEM_EXTEND);
            // 同层节点最后一个更改线的状态
            parentPack.children('.' + CONST$5.ELEM_SET).last().children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).last().addClass(CONST$5.ELEM_LINE_SHORT);
          } else {
            elem.children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).addClass(CONST$5.ELEM_LINE_SHORT);
          }
        } else {
          // 添加延伸线
          if (!packCont.hasClass(CONST$5.ELEM_EXTEND)) {
            packCont.addClass(CONST$5.ELEM_EXTEND);
          }
          // 子节点添加延伸线
          elem.find('.' + CONST$5.ELEM_PACK).each(function () {
            $(this).children('.' + CONST$5.ELEM_SET).last().addClass(CONST$5.ELEM_LINE_SHORT);
          });
          // 如果前一个节点有延伸线
          if (packCont.children('.' + CONST$5.ELEM_SET).last().prev().hasClass(CONST$5.ELEM_LINE_SHORT)) {
            packCont.children('.' + CONST$5.ELEM_SET).last().prev().removeClass(CONST$5.ELEM_LINE_SHORT);
          } else {
            // 若之前的没有，说明处于连接状态
            packCont.children('.' + CONST$5.ELEM_SET).last().removeClass(CONST$5.ELEM_LINE_SHORT);
          }
          // 若是最外层，要始终保持相连的状态
          if (!elem.parent('.' + CONST$5.ELEM_PACK)[0] && elem.next()[0]) {
            packCont.children('.' + CONST$5.ELEM_SET).last().removeClass(CONST$5.ELEM_LINE_SHORT);
          }
        }
      }
      ...
      if (elemMain.find('input[same="layuiTreeCheck"]')[0].checked) {
        var packLast = elem.children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).last();
      ...
      var text = elemMain.children('.' + CONST$5.ELEM_TEXT).html();
      elemMain.children('.' + CONST$5.ELEM_TEXT).html('');
      // 添加输入框，覆盖在文字上方
      ...
      var getVal = function (input) {
        ...
        input.remove();
        elemMain.children('.' + CONST$5.ELEM_TEXT).html(textNew);
      ...
      layer.confirm(i18nText, function (index) {
        ...
        // 若删除最后一个，显示空数据提示
        if (!elem.prev('.' + CONST$5.ELEM_SET)[0] && !elem.next('.' + CONST$5.ELEM_SET)[0] && !elem.parent('.' + CONST$5.ELEM_PACK)[0]) {
          elem.remove();
          that.elem.append(that.elemNone);
          return;
        }
        // 若有兄弟节点
        if (elem.siblings('.' + CONST$5.ELEM_SET).children('.' + CONST$5.ELEM_ENTRY)[0]) {
          // 若开启复选框
          if (options.showCheckbox) {
            // 若开启复选框，进行下步操作
            var elemDel = function (elem) {
              // 若无父结点，则不执行
              if (!elem.parents('.' + CONST$5.ELEM_SET)[0]) return;
              var siblingTree = elem.siblings('.' + CONST$5.ELEM_SET).children('.' + CONST$5.ELEM_ENTRY);
              var parentTree = elem.parent('.' + CONST$5.ELEM_PACK).prev();
              ...
              if (checkState.checked == false) {
                ...
                if (state == 1 && num == 1) {
                  ...
                  that.renderForm('checkbox');
                  // 向上遍历祖先节点
                  elemDel(parentTree.parent('.' + CONST$5.ELEM_SET));
                }
              }
            };
            elemDel(elem);
          ...
          if (options.showLine) {
            ...
            if (num == 1) {
              ...
              // 若为最后一个节点
              if (!elem.next()[0]) {
                elem.prev().children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).last().addClass(CONST$5.ELEM_LINE_SHORT);
              } else {
                parentPack.children('.' + CONST$5.ELEM_SET).last().children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).last().addClass(CONST$5.ELEM_LINE_SHORT);
              }
              // 若为最外层最后一个节点，去除前一个结点的连接线
              if (!elem.next()[0] && !elem.parents('.' + CONST$5.ELEM_SET)[1] && !elem.parents('.' + CONST$5.ELEM_SET).eq(0).next()[0]) {
                elem.prev('.' + CONST$5.ELEM_SET).addClass(CONST$5.ELEM_LINE_SHORT);
              }
            } else {
              // 若为最后一个节点且有延伸线
              if (!elem.next()[0] && elem.hasClass(CONST$5.ELEM_LINE_SHORT)) {
                elem.prev().addClass(CONST$5.ELEM_LINE_SHORT);
              }
            }
          }
        } else {
          // 若无兄弟节点
          var prevDiv = elem.parent('.' + CONST$5.ELEM_PACK).prev();
          // 若开启了连接线
          if (options.showLine) {
            prevDiv.find('.' + CONST$5.ICON_CLICK).removeClass('layui-tree-icon');
            prevDiv.find('.' + CONST$5.ICON_CLICK).children('.layui-icon').removeClass(CONST$5.ICON_SUB).addClass('layui-icon-leaf');
            // 父节点所在层添加延伸线
            var pare = prevDiv.parents('.' + CONST$5.ELEM_PACK).eq(0);
            pare.addClass(CONST$5.ELEM_EXTEND);

            // 兄弟节点最后子节点添加延伸线
            pare.children('.' + CONST$5.ELEM_SET).each(function () {
              $(this).children('.' + CONST$5.ELEM_PACK).children('.' + CONST$5.ELEM_SET).last().addClass(CONST$5.ELEM_LINE_SHORT);
            });
          } else {
            // 父节点隐藏箭头
            prevDiv.find('.layui-tree-iconArrow').addClass(CONST$5.CLASS_HIDE);
          }
          // 移除展开属性
          elem.parents('.' + CONST$5.ELEM_SET).eq(0).removeClass(CONST$5.ELEM_SPREAD);
          // 移除节点容器
          elem.parent('.' + CONST$5.ELEM_PACK).remove();
        }
        elem.remove();
      });
    }
...
// 部分事件
Class$3.prototype.events = function () {
  var that = this;
  var options = that.config;
  ...
  that.elem.find('.layui-tree-search').on('keyup', function () {
    ...
    // 遍历所有的值
    pack.find('.' + CONST$5.ELEM_TEXT).each(function () {
      var entry = $(this).parents('.' + CONST$5.ELEM_ENTRY);
      // 若值匹配，加一个类以作标识
      if ($(this).html().indexOf(val) != -1) {
        ...
        var select = function (div) {
          ...
          // 向上父节点渲染
          if (div.parent('.' + CONST$5.ELEM_PACK)[0]) {
            select(div.parent('.' + CONST$5.ELEM_PACK).parent('.' + CONST$5.ELEM_SET));
          }
        };
        select(entry.parent('.' + CONST$5.ELEM_SET));
      }
    });

    // 根据标志剔除
    pack.find('.' + CONST$5.ELEM_ENTRY).each(function () {
      var parent = $(this).parent('.' + CONST$5.ELEM_SET);
      if (!parent.hasClass('layui-tree-searchShow')) {
        parent.addClass(CONST$5.CLASS_HIDE);
      }
    });
  ...
  that.elem.find('.layui-tree-search').on('keydown', function () {
    $(this).nextAll().find('.' + CONST$5.ELEM_ENTRY).each(function () {
      var parent = $(this).parent('.' + CONST$5.ELEM_SET);
      parent.removeClass('layui-tree-searchShow ' + CONST$5.CLASS_HIDE);
    });
...
// 得到选中节点
Class$3.prototype.getChecked = function () {
  var that = this;
  var options = that.config;
  var customName = options.customName;
  var checkedId = [];
...
// 设置选中节点
Class$3.prototype.setChecked = function (checkedId) {
  var that = this;
  var options = that.config;
  ...
  // 初始选中状态
  that.elem.find('.' + CONST$5.ELEM_SET).each(function (i) {
    var thisId = $(this).data('id');
    var input = $(this).children('.' + CONST$5.ELEM_ENTRY).find('input[same="layuiTreeCheck"]');
...
$.extend(component$4, {
  ...
  getChecked: function (id) {
    var that = component$4.getInst(id);
    if (!that) return;
...
var component$3 = component$e({
  ...
  render: function () {
    ...
    // 各级容器
    that.layBox = that.elem.find('.' + CONST$4.ELEM_BOX);
    that.layHeader = that.elem.find('.' + CONST$4.ELEM_HEADER);
    that.laySearch = that.elem.find('.' + CONST$4.ELEM_SEARCH);
    that.layData = thisElem.find('.' + CONST$4.ELEM_DATA);
    that.layBtn = thisElem.find('.' + CONST$4.ELEM_ACTIVE + ' .layui-btn');
...
var CONST$4 = component$3.CONST;

/**
 * 扩展组件原型方法
 */

var Class$2 = component$3.Class;

// 渲染数据
Class$2.prototype.renderData = function () {
  var that = this;
  var options = that.config;
...
// 渲染表单
Class$2.prototype.renderForm = function (type) {
  form.render(type, 'LAY-transfer-' + this.index);
};

// 同步复选框和按钮状态
Class$2.prototype.renderCheckBtn = function (obj) {
  var that = this;
  var options = that.config;
  obj = obj || {};
  that.layBox.each(function (_index) {
    var othis = $(this);
    var thisDataElem = othis.find('.' + CONST$4.ELEM_DATA);
    var allElemCheckbox = othis.find('.' + CONST$4.ELEM_HEADER).find('input[type="checkbox"]');
    ...
    that.layBtn.eq(_index)[haveChecked ? 'removeClass' : 'addClass'](CONST$4.BTN_DISABLED); // 对应的按钮状态

    // 无数据视图
    if (!obj.stopNone) {
      var isNone = thisDataElem.children('li:not(.' + CONST$4.CLASS_HIDE + ')').length;
...
// 无数据视图
Class$2.prototype.noneView = function (thisDataElem, text) {
  var createNoneElem = $('<p class="layui-none">' + (text || '') + '</p>');
  if (thisDataElem.find('.' + CONST$4.CLASS_NONE)[0]) {
    thisDataElem.find('.' + CONST$4.CLASS_NONE).remove();
  }
  text.replace(/\s/g, '') && thisDataElem.append(createNoneElem);
};

// 同步 value 属性值
Class$2.prototype.setValue = function () {
  var that = this;
  var options = that.config;
  var arr = [];
  that.layBox.eq(1).find('.' + CONST$4.ELEM_DATA + ' input[type="checkbox"]').each(function () {
    var isHide = $(this).data('hide');
...
// 解析数据
Class$2.prototype.parseData = function (callback) {
  var that = this;
  var options = that.config;
  var newData = [];
...
// 获得右侧面板数据
Class$2.prototype.getData = function (value) {
  var that = this;
  var options = that.config;
  var selectedData = [];
  that.setValue();
...
// 执行穿梭
Class$2.prototype.transfer = function (_index, elem) {
  var that = this;
  var options = that.config;
    ...
    thisBoxElem.each(function () {
      ...
      thisDataElem.children('li').each(function () {
        ...
        if (thisElemCheckbox[0].checked && !isHide) {
          ...
          thisBoxElem.siblings('.' + CONST$4.ELEM_BOX).find('.' + CONST$4.ELEM_DATA).append(thisList.clone());
          thisList.remove();
  ...
  var siblingInput = thisBoxElem.siblings('.' + CONST$4.ELEM_BOX).find('.' + CONST$4.ELEM_SEARCH + ' input');
...
Class$2.prototype.events = function () {
  ...
  that.elem.on('click', 'input[lay-filter="layTransferCheckbox"]+', function () {
    ...
    var thisDataElem = thisElemCheckbox.parents('.' + CONST$4.ELEM_BOX).eq(0).find('.' + CONST$4.ELEM_DATA);
  ...
  // 双击穿梭
  that.elem.on('dblclick', '.' + CONST$4.ELEM_DATA + '>li', function () {
    var elemThis = $(this);
  ...
  that.layBtn.on('click', function () {
    ...
    if (othis.hasClass(CONST$4.BTN_DISABLED)) return;
  ...
  that.laySearch.find('input').on('keyup', function () {
    var value = this.value;
    var thisDataElem = $(this).parents('.' + CONST$4.ELEM_SEARCH).eq(0).siblings('.' + CONST$4.ELEM_DATA);
    ...
    thisListElem.each(function () {
      ...
      thisList[isMatch ? 'removeClass' : 'addClass'](CONST$4.CLASS_HIDE);
      thisElemCheckbox.data('hide', isMatch ? false : true);
    });
    that.renderCheckBtn();

    // 无匹配数据视图
    var isNone = thisListElem.length === thisDataElem.children('li.' + CONST$4.CLASS_HIDE).length;
...
$.extend(component$3, {
  // 获得选中的数据（右侧面板）
  getData: function (id) {
    var that = component$3.getInst(id);
    if (!that) return;
    return that.getData();
  }
});
...
var component$2 = component$e({
  ...
  render: function () {
    var that = this;
    var options = that.config;
    that.elemItem = options.elem.find(CONST$3.ELEM_ITEM);
    if (options.index < 0) {
      options.index = 0;
    }
    ...
    // 初始焦点状态
    that.elemItem.eq(options.index).addClass(CONST$3.CLASS_THIS);
...
var CONST$3 = component$2.CONST;

/**
 * 扩展组件原型方法
 */

var Class$1 = component$2.Class;

// 获取上一个等待条目的索引
Class$1.prototype.prevIndex = function () {
  var that = this;
  var options = that.config;
...
// 获取下一个等待条目的索引
Class$1.prototype.nextIndex = function () {
  var that = this;
  var options = that.config;
...
// 索引递增
Class$1.prototype.addIndex = function (num) {
  var that = this;
  var options = that.config;
  num = num || 1;
...
// 索引递减
Class$1.prototype.subIndex = function (num) {
  var that = this;
  var options = that.config;
  num = num || 1;
  options.index = options.index - num;
...
// 自动轮播
Class$1.prototype.autoplay = function () {
  var that = this;
  var options = that.config;
...
// 箭头
Class$1.prototype.arrow = function () {
  var that = this;
  var options = that.config;
  var itemsCount = that.elemItem.length;

  // 模板
  var tplArrow = $(['<button type="button" class="layui-icon ' + (options.anim === 'updown' ? 'layui-icon-up' : 'layui-icon-left') + ' ' + CONST$3.ELEM_ARROW + '" lay-type="sub"></button>', '<button type="button" class="layui-icon ' + (options.anim === 'updown' ? 'layui-icon-down' : 'layui-icon-right') + ' ' + CONST$3.ELEM_ARROW + '" lay-type="add"></button>'].join(''));

  // 预设基础属性
  options.elem.attr('lay-arrow', options.arrow);

  // 避免重复插入
  if (options.elem.find('.' + CONST$3.ELEM_ARROW)[0]) {
    options.elem.find('.' + CONST$3.ELEM_ARROW).remove();
  }
...
// 跳转到特定下标
Class$1.prototype.goto = function (index) {
  var that = this;
  var options = that.config;
...
// 指示器
Class$1.prototype.indicator = function () {
  var that = this;
  var options = that.config;
  var itemsCount = that.elemItem.length;

  // 模板
  var tplInd = that.elemInd = $(['<div class="' + CONST$3.ELEM_IND + '"><ul>', function () {
    var li = [];
  ...
  // 避免重复插入
  if (options.elem.find('.' + CONST$3.ELEM_IND)[0]) {
    options.elem.find('.' + CONST$3.ELEM_IND).remove();
  }
...
// 滑动切换
Class$1.prototype.slide = function (type, num) {
  var that = this;
  ...
  if (type === 'sub') {
    that.subIndex(num);
    elemItem.eq(options.index).addClass(CONST$3.ELEM_PREV);
    setTimeout(function () {
      elemItem.eq(thisIndex).addClass(CONST$3.ELEM_RIGHT);
      elemItem.eq(options.index).addClass(CONST$3.ELEM_RIGHT);
    }, 50);
  } else {
    // 默认递增滑
    that.addIndex(num);
    elemItem.eq(options.index).addClass(CONST$3.ELEM_NEXT);
    setTimeout(function () {
      elemItem.eq(thisIndex).addClass(CONST$3.ELEM_LEFT);
      elemItem.eq(options.index).addClass(CONST$3.ELEM_LEFT);
    }, 50);
  }

  // 移除过渡类
  setTimeout(function () {
    elemItem.removeClass(CONST$3.CLASS_THIS + ' ' + CONST$3.ELEM_PREV + ' ' + CONST$3.ELEM_NEXT + ' ' + CONST$3.ELEM_LEFT + ' ' + CONST$3.ELEM_RIGHT);
    elemItem.eq(options.index).addClass(CONST$3.CLASS_THIS);
    that.haveSlide = false; // 解锁
  }, 350);

  // 指示器焦点
  that.elemInd.find('li').eq(options.index).addClass(CONST$3.CLASS_THIS).siblings().removeClass(CONST$3.CLASS_THIS);
  that.haveSlide = true;
  ...
  layui.event.call(this, CONST$3.MOD_NAME, 'change(' + filter + ')', params);
};
...
var component$1 = component$e({
  ...
  render: function () {
    ...
    for (var i = 1; i <= options.length; i++) {
      var item = '<li class="layui-inline"><i class="layui-icon ' + (i > Math.floor(options.value) ? CONST$2.ICON_RATE : CONST$2.ICON_RATE_SOLID) + '" ' + style + '></i></li>';
    ...
    // 开始插入替代元素
    var othis = options.elem;
    var hasRender = othis.next('.' + CONST$2.ELEM);
...
var CONST$2 = component$1.CONST;

/**
 * 扩展组件原型方法
 */

var Class = component$1.Class;
  ...
  liElems.each(function (index) {
    ...
    othis.on('mousemove', function (e) {
      _ul.find('i').each(function () {
        $(this).addClass(CONST$2.ICON_RATE).removeClass(CONST$2.ICON_SOLID_HALF);
      });
      _ul.find('i:lt(' + ind + ')').each(function () {
        $(this).addClass(CONST$2.ICON_RATE_SOLID).removeClass(CONST$2.ICON_HALF_RATE);
      });
      // 如果设置可选半星，那么判断鼠标相对 li 的位置
      if (options.half) {
        var x = e.pageX - $(this).offset().left;
        if (x <= wide / 2) {
          othis.children('i').addClass(CONST$2.ICON_RATE_HALF).removeClass(CONST$2.ICON_RATE_SOLID);
        }
      }
    });
    ...
    othis.on('mouseleave', function () {
      ...
      if (options.half) {
        if (parseInt(options.value) !== options.value) {
          _ul.children('li:eq(' + Math.floor(options.value) + ')').children('i').addClass(CONST$2.ICON_RATE_HALF).removeClass(CONST$2.ICON_SOLID_RATE);
        }
      }
    });
  });
  lay.touchSwipe(_ul, {
    onTouchMove: function (e, state) {
      ...
      liElems.each(function (index) {
        ...
        if (needSelect) {
          // 设置选中样式
          iconElem.addClass(CONST$2.ICON_RATE_SOLID).removeClass(CONST$2.ICON_HALF_RATE);
          if (options.half && shouldHalfIcon) {
            iconElem.addClass(CONST$2.ICON_RATE_HALF).removeClass(CONST$2.ICON_RATE_SOLID);
          }
        } else {
          // 恢复初始样式
          iconElem.addClass(CONST$2.ICON_RATE).removeClass(CONST$2.ICON_SOLID_HALF);
        }
...
var component = component$e({
  ...
  render: function () {
    ...
    var $more = $('<div class="' + CONST$1.ELEM_MORE + '"><a href="javascript:;">' + ELEM_TEXT + '</a></div>');
    elem.find('.' + CONST$1.ELEM_MORE).remove(); // 清除旧的「加载更多」元素
    ...
    var done = function fn() {
      locked = true;
      $moreBtn.html(CONST$1.ELEM_LOAD);
    ...
    // 滚动条滚动事件
    var timer;
    var FLOW_SCROLL_EVENTS = CONST$1.FLOW_SCROLL_EVENTS + '_' + options.id;
...
$.extend(component, {
  ...
  lazyimg: function (options) {
    ...
    var timer;
    var id = options.id || '';
    var LAZYIMG_SCROLL_EVENTS = CONST$1.LAZYIMG_SCROLL_EVENTS + '_' + id;
...
function code(options, mode) {
  ...
  if (options.preview) {
    ...
    // tab change
    component$6.on('tab(' + FILTER_VALUE + ')', function (data) {
      var $this = $(this);
